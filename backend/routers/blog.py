...
async def admin_review_post(post_id: str, body: BlogReviewBody, user=Depends(require_owner)):
    ...
    # ── Push broadcast (Phase 4, 2026-05-31) ──────────────────────
    # When the owner publishes a post, fan out a push to every patient
    # who has registered a device — keeps Education content fresh.
    # Best-effort; never block the response on broadcast failure.
    if new_status == "published":
        try:
            post = await db.blog_posts.find_one(
                {"post_id": post_id},
                {"_id": 0, "post_id": 1, "title": 1, "slug": 1, "summary": 1, "language": 1},
            ) or {}
            title = (post.get("title") or "").strip() or "New article"
            summary = (post.get("summary") or "").strip()[:140]
            slug = post.get("slug") or post.get("post_id") or post_id
            # Fan-out to all patient-role users — capped at 2,000 for
            # safety. The Emergent push relay handles users without an
            # active token silently (logs but doesn't fail), so we no
            # longer need to pre-filter by `push_tokens.0` on the user
            # doc. Every patient gets the in-app bell + a push attempt.
            ...
            async for u in cursor:
                try:
                    await create_notification(
                        ...
                        body=summary or "A new post has been published by the clinic.",
                        kind="blog_published",
                        data={
                            "type": "blog_published",
                            "post_id": post_id,
                            "slug": slug,
                            "deep_link": f"/blog/{slug}",
                        },
                        push=True,
                    )
                    count += 1
