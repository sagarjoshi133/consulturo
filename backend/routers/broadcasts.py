...
async def create_broadcast(request: Request, payload: BroadcastCreate, user=Depends(require_staff)):
    ...
    if doc["status"] == "pending_approval":
        ...
        if approver_uids:
            # Route via the Emergent relay (production transport);
            # fall back to the legacy Expo-direct path in dev.
            from services.push_relay import send_push as _relay_send, is_configured as _relay_on
            if _relay_on():
                await _relay_send(
                    recipients=approver_uids,
                    data={
                        "title": "Broadcast awaiting approval",
                        "message": f"{doc['author_name']}: {title}",
                        "type": "broadcast_review",
                        "broadcast_id": bid,
                    },
                    kind="broadcast_review",
                )
            else:
...
async def review_broadcast(bid: str, body: BroadcastReview, user=Depends(require_user)):
    ...
    if send_now:
        # Gather target user_ids and route through the Emergent push
        # relay (`send_push`) — this handles token resolution upstream,
        # so a user with multiple devices gets all of them in one call
        # and we no longer rely on our local `push_tokens` cache.
        ...
        if _relay_on() and uids:
            res = await _relay_send(
                ...
                kind="broadcast",
                idempotency_key=f"broadcast-{bid}",
            )
        else:
            # Fallback: legacy Expo-direct path (dev / no relay key).
