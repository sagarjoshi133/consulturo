...
from db import db
from auth_deps import OWNER_TIER_ROLES, PRIMARY_TIER_ROLES, STAFF_ROLES, VALID_ROLES, is_super_owner, require_owner, require_user
...
def _actor_can_grant_blog(actor: Dict[str, Any]) -> bool:
    """Return True iff `actor` is allowed to grant blog-writing rights
    to a team member. Super-owner can always grant. Owner-tier roles
    (primary_owner / owner / partner) can grant only if their own blog
    access has NOT been revoked by the super-owner (default True)."""
    if is_super_owner(actor):
        return True
    if actor.get("role") in ("primary_owner", "owner", "partner"):
        return actor.get("can_create_blog") is not False
    return False
...
async def create_invite(body: TeamInviteBody, user=Depends(require_owner)):
    ...
    # Blog-grant guard — only an actor who themselves has blog access
    # may pass `can_manage_blog: true` for a team member. Silently
    # downgrade if the actor lacks the privilege so the invite still
    # succeeds for the other abilities.
    grant_blog = bool(body.can_manage_blog) and _actor_can_grant_blog(user)
    if bool(body.can_manage_blog) and not grant_blog:
        raise HTTPException(
            status_code=403,
            detail="Your account doesn't have blog access, so you can't grant it to others.",
        )
    invite_doc = {
        "email": email_l,
        "name": body.name,
        ...
        "can_prescribe": bool(body.can_prescribe),
        "can_manage_surgeries": bool(body.can_manage_surgeries),
        "can_manage_availability": bool(body.can_manage_availability),
        "can_manage_billing": bool(body.can_manage_billing),
        "can_manage_ipd": bool(body.can_manage_ipd),
        "can_view_analytics": bool(body.can_view_analytics),
        "can_manage_blog": grant_blog,
        "can_manage_settings": bool(body.can_manage_settings),
...
async def update_team_member(email: str, body: TeamUpdateBody, user=Depends(require_owner)):
    ...
    # ── New permission gates (Session 5.9) ─────────────────────
    if body.can_manage_billing is not None:
        updates["can_manage_billing"] = bool(body.can_manage_billing)
    if body.can_manage_ipd is not None:
        updates["can_manage_ipd"] = bool(body.can_manage_ipd)
    if body.can_view_analytics is not None:
        updates["can_view_analytics"] = bool(body.can_view_analytics)
    if body.can_manage_blog is not None:
        # Guard — only an actor who themselves has blog access can
        # grant it. We allow them to REVOKE (set False) at any time.
        if bool(body.can_manage_blog) and not _actor_can_grant_blog(user):
            raise HTTPException(
                status_code=403,
                detail="Your account doesn't have blog access, so you can't grant it to others.",
            )
        updates["can_manage_blog"] = bool(body.can_manage_blog)
    if body.can_manage_settings is not None:
        updates["can_manage_settings"] = bool(body.can_manage_settings)
...
async def list_team(user=Depends(require_owner)):
    ...
    for iv in invites:
        by_email[iv["email"]] = {
            ...
            "can_manage_billing": iv.get("can_manage_billing", False),
            "can_manage_ipd": iv.get("can_manage_ipd", False),
            "can_view_analytics": iv.get("can_view_analytics", False),
            "can_manage_blog": iv.get("can_manage_blog", False),
            "can_manage_settings": iv.get("can_manage_settings", False),
            "status": "invited",
        }
    ...
    for u in users:
        ...
        if role in STAFF_ROLES or role in custom_slugs:
            by_email[u["email"]] = {
                ...
                # New ability gates default to owner-tier yes / staff no.
                # Reception is typically given billing access manually
                # so we don't auto-grant it here.
                "can_manage_billing": bool(u.get("can_manage_billing", role in OWNER_TIER_ROLES)),
                "can_manage_ipd": bool(u.get("can_manage_ipd", role in OWNER_TIER_ROLES)),
                "can_view_analytics": bool(u.get("can_view_analytics", role in OWNER_TIER_ROLES)),
                "can_manage_blog": bool(u.get("can_manage_blog", role in OWNER_TIER_ROLES)),
                "can_manage_settings": bool(u.get("can_manage_settings", role in OWNER_TIER_ROLES)),
