...
async def create_prescription(request: Request, payload: PrescriptionCreate, user=Depends(require_staff)):
    ...
    # Notify the patient when a FINAL Rx is issued. Drafts stay
    # silent — they're a work-in-progress. Best-effort; never block
    # the Rx-create response on a push failure.
    if status == "final":
        try:
            from services.notifications import create_notification
            patient_phone = (payload.patient_phone or "").strip() or None
            patient_email = getattr(payload, "patient_email", None)
            patient_email = patient_email.strip() if patient_email else None
            first = (payload.patient_name or "").strip().split(" ")[0] or "Patient"
            await create_notification(
                ...
                body=(
                    f"Hi {first}, Dr. Sagar Joshi has issued your prescription. "
                    "Tap to view & download the PDF."
                ),
                kind="prescription_ready",
                data={
                    "type": "prescription_ready",
                    "prescription_id": prescription_id,
                    "deep_link": f"/prescriptions/{prescription_id}",
                },
                push=True,
            )
            # Google review auto-nudge (Phase 5 — June 2026).
            try:
...
async def my_prescriptions(user=Depends(require_user)):
    # A patient may have multiple phone numbers across bookings; surface
    # all rxs by user_id OR phone match. Legacy accounts created before
    # `phone_digits` shipped fall back to digits extracted from the raw
    # `phone` field so My Records still lists everything correctly.
    q: Dict[str, Any] = {"$or": [{"patient_user_id": user["user_id"]}]}
    digits = (user.get("phone_digits") or "").strip()
    if not digits:
    ...
    if reg_no:
        q["$or"].append({"registration_no": reg_no})
...
async def get_prescription(prescription_id: str, user=Depends(require_user)):
    ...
    (matched by user_id, by registration number, OR by phone digits — same
    matching rules as /api/prescriptions/me to keep "My Records" → "Open"
    consistent)."""
    ...
    if role not in STAFF_ROLES and role not in {"prescriber", "doctor"}:
        ...
        reg_no = (user.get("registration_no") or "").strip()
        # Derive last-10 phone digits from `phone_digits` if present
        # (modern users), else fall back to extracting digits from the
        # raw `phone` field (legacy users created before phone_digits
        # was added in the auth router).
        user_digits = (user.get("phone_digits") or "").strip()
        if not user_digits:
            user_digits = "".join(ch for ch in (user.get("phone") or "") if ch.isdigit())
        # Patient can view ONLY their own Rx — check by linked user_id,
        # by patient registration number, OR by phone digits (last 10
        # of the user's phone matching the Rx patient_phone). Phone-
        # based matching is needed because most prescriptions are
        # created before the patient signs up, so `patient_user_id` is
        # empty until the patient self-links.
        rx_uid = doc.get("user_id") or doc.get("patient_user_id")
        rx_reg = (doc.get("registration_no") or "").strip()
        rx_phone_digits = "".join(ch for ch in (doc.get("patient_phone") or "") if ch.isdigit())
        owns = (
            (uid and rx_uid and uid == rx_uid)
            or (reg_no and rx_reg and reg_no == rx_reg)
            or (
                user_digits
                and rx_phone_digits
                and user_digits[-10:] == rx_phone_digits[-10:]
            )
        )
        if not owns:
            raise HTTPException(status_code=404, detail="Not found")
