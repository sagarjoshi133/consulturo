...
async def list_primary_owners(user=Depends(require_owner)):
    ...
    async for u in db.users.find({"role": {"$in": ["primary_owner", "super_owner"]}}, {"_id": 0}):
        ...
        rows.append({
            ...
            # Default-True semantics: a primary_owner has blog access
            # unless the super-owner explicitly revoked it. Super-owner
            # is always-on.
            "can_create_blog": (
                True if u.get("role") == "super_owner"
                else (u.get("can_create_blog") is not False)
            ),
