            ...
            "tokens": [
                {
                    ...
                    "token_preview": (t.get("token") or t.get("device_token") or "")[:30] + "…",
                }
                for t in toks
            ],
        })
...
async def push_self_test(user=Depends(require_user)):
    """Fire a test push to the calling user's devices.

    PRIMARY transport — the Emergent push relay (send by user_id; the
    relay resolves native FCM/APNs tokens upstream). This matches the
    production registration path: devices register native tokens via
    /api/register-push which forwards them to the relay.

    FALLBACK transport — the legacy direct-to-Expo path (only useful
    for old Expo-token rows; works in Expo Go / dev). Used when
    EMERGENT_PUSH_KEY is not configured (preview env) — in that case
    the response also explains that production pushes require the
    deployed app.
    """
    import asyncio as _asyncio
    import re as _re
    from services.push_relay import (
        send_push as _relay_send,
        is_configured as _relay_on,
    )

    uid = user["user_id"]

    # ── PRIMARY: Emergent relay (production path) ────────────────────
    if _relay_on():
        # Count locally-mirrored device rows for diagnostics (native
        # rows store `device_token`; legacy Expo rows store `token`).
        device_rows = await db.push_tokens.count_documents({"user_id": uid})
        res = await _relay_send(
            recipients=[uid],
            data={
        ...
        try:
            ...
            pass
        ok = (res.get("sent") or 0) > 0 and not res.get("errors")
        msg = (
            "Test push sent via the Emergent relay. It should arrive on your phone within a few seconds."
            if ok
            else "The push relay rejected the send — see errors."
        )
        if ok and device_rows == 0:
            msg += (
                " Note: no devices have registered from this account in this "
                "environment yet — make sure you opened the installed app and "
                "granted notification permission."
            )
        return {
            "ok": ok,
            "transport": "emergent_relay",
            "tokens_found": device_rows,
            "sent": res.get("sent"),
    ...
    if not tokens:
        ...
        return {
            "ok": False,
            "reason": "no_tokens",
            "transport": "expo_direct_fallback",
            "message": (
                "No push tokens registered for this account. "
                "Note: this environment has no EMERGENT_PUSH_KEY (preview) — "
                "production pushes only work in the deployed app. "
                "Grant notification permission in the installed app and reopen it."
            ),
            "tokens_found": 0,
            "diagnostics": diag,
        }
