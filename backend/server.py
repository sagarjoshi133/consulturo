    ...
    async def _loop():
        while True:
            try:
                ...
                # ---- Surgery reminders (5 days before scheduled_date) ----
                try:
                    await _scan_and_fire_surgery_5d_reminders(now)
                except Exception:
                    pass

                # ---- Video pre-call reminders (T-N min and T-5 min nudge) ----
                try:
                    await _scan_and_fire_video_precall_reminders(now)
                except Exception:
                    pass

                # ---- Video no-show auto-detection (T+grace min) ----
                try:
                    await _scan_and_mark_video_no_shows(now)
                except Exception:
                    pass

                # ---- Google review nudges (T+delay_hours after event) ----
                try:
                    from services.review_request import fire_due_review_requests
...
async def _scan_and_fire_surgery_5d_reminders(now: datetime) -> None:
    """Fire WhatsApp + push reminder 5 days before a scheduled surgery.

    Behaviour:
      • Targets `surgeries` with `scheduled_date` exactly 5 days from
        today (IST). Window: 4.5d–5.5d so a 60-second-loop catches it
        regardless of cron drift.
      • Each surgery is reminded ONCE; we set `reminder_5d_fired_at`
        as a guard. Surgeries with `surgery_status="cancelled"` or
        `"completed"` are skipped.
      • Channels:
         (a) In-app + push notification to the patient (if linked).
         (b) Telegram alert to the clinic-owner with a `wa.me` link
             pre-filled with the reminder text so the doctor/staff
             can WhatsApp the patient in one tap.
    """
    today_ist = (now + IST_OFFSET).date()
    # 5-day window: scheduled_date == today_ist + 5
    target_date = (today_ist + timedelta(days=5)).isoformat()

    cursor = db.surgeries.find({
        "scheduled_date": target_date,
        "surgery_status": {"$nin": ["cancelled", "completed"]},
        "reminder_5d_fired_at": {"$exists": False},
    }).limit(200)

    async for sx in cursor:
        try:
            patient_name = (sx.get("patient_name") or "").strip() or "Patient"
            first_name = patient_name.split(" ")[0]
            sched_date = sx.get("scheduled_date") or ""
            sched_time = (sx.get("scheduled_time") or "").strip() or ""
            ...
            # (a) In-app + push to patient if linked
            if patient_user_id:
                try:
                    await create_notification(
                        user_id=patient_user_id,
                        title="🗓️ Surgery in 5 days",
                        body=patient_body,
                        kind="surgery_reminder",
                        data={
                            "type": "surgery_reminder",
                            "surgery_id": sx.get("surgery_id"),
                            "window": "5d",
                        },
                        push=True,
                    )
            ...
            # (b) Telegram alert with wa.me link for one-tap WhatsApp
            try:
                _digits = re.sub(r"\D", "", patient_phone)
                if _digits:
                    _wa_digits = _digits if len(_digits) > 10 else ("91" + _digits)
                    _wa_link = f"https://wa.me/{_wa_digits}?text={_urlencode(patient_body)}"
                    _msg = (
                        "🗓️ <b>SURGERY REMINDER — 5 days</b>\n"
                        f"👤 <b>{htmllib.escape(patient_name)}</b>\n"
                        f"📞 {htmllib.escape(patient_phone)}\n"
                        f"🔪 {htmllib.escape(surgery_name)}\n"
                        f"📅 {pretty_date}{' · 🕘 ' + sched_time if sched_time else ''}"
                        f"{' · ' + ot_room if ot_room else ''}\n"
                        f"🆔 <code>{sx.get('surgery_id', '')}</code>\n"
                        f'<a href="{_wa_link}">📲 Send WhatsApp reminder</a>'
                    )
                    await notify_telegram(_msg)
            except Exception:
                pass

            # Mark as fired so we never double-send
            await db.surgeries.update_one(
                {"_id": sx["_id"]},
                {"$set": {
                    "reminder_5d_fired_at": now,
                    "reminder_5d_target_date": target_date,
                }},
            )
        except Exception:
            # Never let a single surgery crash the loop
            continue


async def _scan_and_fire_video_precall_reminders(now: datetime) -> None:
    """Pre-call reminders for video consultations.

    Two reminder windows per booking — each fired AT MOST ONCE:
      1. T-N min   (N = clinic_settings.video.pre_call_reminder_minutes,
                    default 5; admin-configurable). Marker:
                    `video_reminder_fired_at`. Skipped if N ≤ 5 (would
                    coincide with the 5-min nudge below).
      2. T-5 min   "final nudge". Marker: `video_5m_nudge_fired_at`.

    For each fire we:
      • push + bell-notify the PATIENT with their join URL,
      • push + bell-notify the DOCTOR (owner-tier) with the doctor URL,
      • Telegram the clinic with one-tap wa.me links pre-filled with
        the patient's join URL (manual WhatsApp delivery — there is no
        WhatsApp Business API key wired into ConsultUro).

    Skipped automatically when:
      • booking is not status="confirmed"
      • mode is not video/online/tele
      • video_room.patient_url is missing (room never provisioned)
      • the booking is in the past (>5 min behind now)
    """
    ...
    pre_min = int(video_cfg.get("pre_call_reminder_minutes", 5) or 5)
    pre_min = max(0, min(120, pre_min))

    # Look at confirmed video bookings whose booking_date is within
    # ±1 day of today (IST). Most reminders fire same-day; the buffer
    # handles edge cases around midnight rollover.
    today_ist = (now + IST_OFFSET).date()
    earliest_iso = (today_ist - timedelta(days=1)).isoformat()
    latest_iso = (today_ist + timedelta(days=1)).isoformat()

    cursor = db.bookings.find({
        "status": "confirmed",
        "mode": {"$in": ["video", "online", "tele"]},
        "booking_date": {"$gte": earliest_iso, "$lte": latest_iso},
        "video_room.patient_url": {"$exists": True, "$ne": ""},
    }).limit(200)
    ...
    async for b in cursor:
        try:
            ...
            try:
                ...
                continue
            booking_ts = datetime(yr, mo, dy, hh, mm, tzinfo=timezone.utc) - IST_OFFSET
            delta = booking_ts - now

            # Skip past bookings (>5 min behind)
            if delta < timedelta(minutes=-5):
                continue
            # Don't even consider bookings further out than pre_min+2 min
            if delta > timedelta(minutes=max(pre_min, 5) + 2):
                continue

            video_room = b.get("video_room") or {}
            patient_url = video_room.get("patient_url") or ""
            doctor_url = video_room.get("doctor_url") or ""
            if not patient_url:
                continue

            booking_id = b.get("booking_id", "")
            patient_name = (b.get("patient_name") or "").strip() or "Patient"
            first_name = patient_name.split(" ")[0]
            patient_phone = (b.get("patient_phone") or "").strip()
            patient_user_id = b.get("user_id")
            when_disp = _format_booking_display(b_date, b_time)

            patient_body = (
                f"Hi {first_name}, your video consultation with "
                f"Dr. Sagar Joshi is starting soon ({when_disp}). "
                f"Tap to join: {patient_url}\n\n"
                "Tip: connect on Wi-Fi and use earphones for best quality."
            )
            doctor_body = (
                f"🎥 Video consult with {patient_name} starts soon "
                f"({when_disp}). Tap to enter the room as host: {doctor_url}"
            )

            # ── Primary reminder window: T-N min ──────────────────
            # Window: [pre_min - 0.5, pre_min + 0.5] minutes from now.
            # If pre_min ≤ 5 we skip — the 5-min nudge will cover it.
            if pre_min > 5 and not b.get("video_reminder_fired_at"):
                lo = timedelta(minutes=pre_min - 1)
                hi = timedelta(minutes=pre_min + 1)
                if lo <= delta <= hi:
                    await _fire_video_reminder(
                        ...
                        window_label=f"{pre_min}m",
                        marker_field="video_reminder_fired_at",
                        now=now,
                    )

            # ── Final 5-min nudge ─────────────────────────────────
            if not b.get("video_5m_nudge_fired_at"):
                if timedelta(minutes=4) <= delta <= timedelta(minutes=6):
                    await _fire_video_reminder(
                        booking_id=booking_id,
                        patient_user_id=patient_user_id,
                        patient_phone=patient_phone,
                        ...
                        doctor_url=doctor_url,
                        patient_body=(
                            f"Hi {first_name}, your video appointment is "
                            f"in ~5 minutes. Tap to join now: {patient_url}"
                        ),
                        doctor_body=(
                            f"⏰ 5-min nudge — your video consult with "
                            f"{patient_name} starts in ~5 min. "
                            f"Host link: {doctor_url}"
                        ),
                        window_label="5m",
                        marker_field="video_5m_nudge_fired_at",
                        now=now,
                    )
        except Exception:
            # Never let a single booking crash the loop
...
async def _fire_video_reminder(
    *,
    booking_id: str,
    ...
    """Dispatch one pre-call reminder fan-out and mark the booking so
    the same window never fires twice. Best-effort across all channels
    — a failure in one (e.g. Telegram outage) MUST NOT block the rest.
    """
    # ── 1. Patient: in-app bell + Expo push ──────────────────────
    try:
        await create_notification(
            user_id=patient_user_id,
            phone=patient_phone or None,
            title=f"🎥 Video consult in {window_label}",
            body=patient_body,
            kind="video_precall_reminder",
            data={
                "type": "video_precall_reminder",
                "booking_id": booking_id,
                "patient_url": patient_url,
                "window": window_label,
            },
    ...
    # Also fire push by phone fallback (covers guest bookings where
    # `user_id` wasn't set when create_notification's lookup failed).
    if not patient_user_id and patient_phone:
        try:
            await push_to_user(
                None,
                patient_phone,
                f"🎥 Video consult in {window_label}",
                patient_body,
                {
                    "type": "video_precall_reminder",
                    "booking_id": booking_id,
    ...
    # ── 2. Doctor / owner: push + bell ───────────────────────────
    try:
        await push_to_owner(
            f"🎥 Patient ready in {window_label}",
            doctor_body,
            {
                "type": "video_precall_reminder_doctor",
                "booking_id": booking_id,
                "doctor_url": doctor_url,
                "patient_url": patient_url,
                "window": window_label,
    ...
    # ── 3. Telegram with one-tap wa.me link (manual WA dispatch) ──
    try:
        _digits = re.sub(r"\D", "", patient_phone or "")
        wa_link = ""
        if _digits:
            _wa_digits = _digits if len(_digits) > 10 else ("91" + _digits)
            wa_link = f"https://wa.me/{_wa_digits}?text={_urlencode(patient_body)}"
        msg = (
            f"🎥 <b>VIDEO PRE-CALL REMINDER — T-{window_label}</b>\n"
            f"👤 <b>{htmllib.escape(patient_name)}</b>\n"
        ...
        if wa_link:
            msg += f'<a href="{wa_link}">📲 Send WhatsApp reminder to patient</a>'
        await notify_telegram(msg)
    except Exception:
        pass

    # ── 4. Mark booking so we never double-fire this window ──────
    try:
        await db.bookings.update_one(
            {"booking_id": booking_id},
            {"$set": {marker_field: now}},
        )
    except Exception:
        pass


async def _scan_and_mark_video_no_shows(now: datetime) -> None:
    """Mark video bookings as `no_show` when the patient never joined.

    Rules:
      • Eligible: status=confirmed, mode∈{video,online,tele},
        video_room provisioned, NOT cancelled, NOT completed.
      • A booking flips to no_show ONCE when:
          (booking_time + no_show_grace_minutes) ≤ now
          AND video_room.patient_joined_at is missing/None.
      • Marker: `no_show_at` timestamp + `status: "no_show"`.
      • Side-effect: Telegram alert to clinic so the doctor can
        decide whether to call the patient back.

    `no_show_grace_minutes` is sourced from
    `clinic_settings.video.no_show_grace_minutes` (default 15).
    """
    try:
        cs = await db.clinic_settings.find_one({}, {"_id": 0}) or {}
    except Exception:
        cs = {}
    video_cfg = (cs.get("video") or {})
    grace_min = int(video_cfg.get("no_show_grace_minutes", 15) or 15)
    grace_min = max(0, min(180, grace_min))

    today_ist = (now + IST_OFFSET).date()
    earliest_iso = (today_ist - timedelta(days=2)).isoformat()
    latest_iso = (today_ist + timedelta(days=1)).isoformat()
    ...
    cursor = db.bookings.find({
        ...
        "booking_date": {"$gte": earliest_iso, "$lte": latest_iso},
        "video_room.patient_url": {"$exists": True, "$ne": ""},
        "video_room.patient_joined_at": {"$in": [None, False]},
        "no_show_at": {"$in": [None, False]},
    }).limit(200)

    async for b in cursor:
        try:
            # Double-check absence of patient_joined_at in nested doc
            if (b.get("video_room") or {}).get("patient_joined_at"):
                continue
            b_date = (b.get("booking_date") or "").strip()
            b_time = (b.get("booking_time") or "10:00").strip()[:5]
            if not b_date:
            ...
            try:
                ...
                continue
            booking_ts = datetime(yr, mo, dy, hh, mm, tzinfo=timezone.utc) - IST_OFFSET
            cutoff = booking_ts + timedelta(minutes=grace_min)
            if now < cutoff:
                continue  # still within grace

            # Skip bookings older than 7 days — those are stale records
            # that aren't worth a Telegram blast.
            if now - booking_ts > timedelta(days=7):
                # Still mark them so we never re-process — but no alert.
                await db.bookings.update_one(
                    {"_id": b["_id"]},
                    {"$set": {
                        "no_show_at": now,
            ...
            booking_id = b.get("booking_id", "")
            patient_name = (b.get("patient_name") or "Patient").strip()
            patient_phone = (b.get("patient_phone") or "").strip()
            when_disp = _format_booking_display(b_date, b_time)

            # Persist no-show status atomically
            await db.bookings.update_one(
                {"_id": b["_id"], "no_show_at": {"$in": [None, False]}},
                {"$set": {
                    "no_show_at": now,
                    "status": "no_show",
                    "no_show_grace_min": grace_min,
                }},
            )

            # Telegram alert to clinic
            try:
                msg = (
                    "👻 <b>VIDEO CONSULT — NO-SHOW</b>\n"
                    f"👤 <b>{htmllib.escape(patient_name)}</b> — "
                    f"{htmllib.escape(patient_phone or '—')}\n"
                    f"🕘 scheduled {htmllib.escape(when_disp)}\n"
                    f"🆔 <code>{booking_id}</code>\n"
                    f"⌛ {grace_min} min grace exhausted — "
                    f"patient never joined."
                )
                # One-tap wa.me to follow up with the patient
                _digits = re.sub(r"\D", "", patient_phone or "")
                if _digits:
                    _wa_digits = _digits if len(_digits) > 10 else ("91" + _digits)
                    _follow = (
                        f"Hi {patient_name.split(' ')[0]}, we noticed you "
                        f"missed your scheduled video consultation. "
                        "Please reply with a convenient time to reschedule. "
                        "— Dr. Sagar Joshi's clinic"
                    )
                    _wa_link = f"https://wa.me/{_wa_digits}?text={_urlencode(_follow)}"
                    msg += f'\n<a href="{_wa_link}">📲 Send follow-up WhatsApp</a>'
                await notify_telegram(msg)
            except Exception:
                pass

            # Notify owner via push so the doctor sees the no-show on
            # their device in real-time too.
            try:
                await push_to_owner(
                    "👻 Patient no-show",
                    f"{patient_name} missed the {when_disp} video consult.",
                    {
                        "type": "video_no_show",
                        "booking_id": booking_id,
                    },
    ...
    if invite:
        ...
        if is_core or is_custom:
            return {
                ...
                "can_manage_billing": invite.get(
                    "can_manage_billing", role in OWNER_TIER_ROLES
                ),
                "can_manage_ipd": invite.get(
                    "can_manage_ipd", role in OWNER_TIER_ROLES
                ),
                "can_view_analytics": invite.get(
                    "can_view_analytics", role in OWNER_TIER_ROLES
                ),
                "can_manage_blog": invite.get(
                    "can_manage_blog", role in OWNER_TIER_ROLES
                ),
                "can_manage_settings": invite.get(
                    "can_manage_settings", role in OWNER_TIER_ROLES
                ),
            }
    return {
        "role": "patient",
...
# Policy (June 2025 update):
#   • super_owner   → always allowed (immutable).
#   • primary_owner → ENABLED BY DEFAULT. Super-owner can revoke
#     editorial access by explicitly setting `can_create_blog: false`
#     on the user record via PATCH /api/admin/primary-owners/{id}/blog-perm.
#     A missing flag is treated as TRUE.
#   • partner       → same default-True semantics as primary_owner
#     (clinical co-equal). May be revoked the same way.
#   • staff/other   → only via explicit `can_manage_blog: true` toggle
#     granted by a primary_owner who themselves has blog access (the
#     /api/team router enforces this guard at grant time).
...
async def _seed_demo_patient_data(user_id: str, email: str, name: str) -> Dict[str, int]:
    ...
    try:
        reg_no = await allocate_reg_no(phone, name=name, email=email)  # type: ignore[name-defined]
    except Exception:
...
from routers.patient_db import router as _patient_db_router
from routers.surgical_consents import router as _surgical_consents_router
...
app.include_router(_patient_db_router)
app.include_router(_surgical_consents_router)
app.include_router(_tools_router)
app.include_router(_admin_users_router)

# ─── Factory Reset (super_owner + primary_owner destructive) ───
from routers.factory_reset import router as _factory_reset_router
app.include_router(_factory_reset_router)

# ─── Platform Reset (super_owner ONLY — wipes every tenant) ───
from routers.platform_reset import router as _platform_reset_router
app.include_router(_platform_reset_router)
...
from routers.receipts import router as _receipts_router
from routers.video import router as _video_router
from routers.assistant import router as _assistant_router
from routers.review_requests import router as _review_requests_router
from routers.featured_reviews import router as _featured_reviews_router
from routers.guides import router as _guides_router
from routers.diets import router as _diets_router
from routers.ipd import router as _ipd_router
from routers.drug_repository import router as _drug_repo_router
from routers.phr import router as _phr_router
from routers.razorpay_pay import router as _razorpay_pay_router
...
app.include_router(_review_requests_router)
app.include_router(_featured_reviews_router)
app.include_router(_guides_router)
app.include_router(_diets_router)
app.include_router(_ipd_router)
app.include_router(_drug_repo_router)
app.include_router(_phr_router)
app.include_router(_razorpay_pay_router)
...
# ─── Session 5.18 — Medical Certificate generator ────────────────────
from routers.medical_certificates import router as _med_certs_router
app.include_router(_med_certs_router)


# ─── Phase 5.12 — AI clinical document drafts ────────────────────────
# Powers AI-assist buttons for Medical Certificate · IPD Progress Note ·
# Discharge Summary. Backed by Emergent universal LLM key (Gemini for
# short-form, Claude Sonnet 4.5 for the multi-page discharge summary).
from routers.ai_clinical import router as _ai_clinical_router
app.include_router(_ai_clinical_router)

# Phase 5.21 — AI Weekly Clinic Summary (executive briefing email
# generated by Claude every Monday morning, plus an on-demand
# dashboard preview endpoint).
from routers.weekly_summary import router as _weekly_summary_router
app.include_router(_weekly_summary_router)

# ─── Wave 1 — Global search, patient timeline, Rx templates,
# allergies, lab trends ───
from routers.wave1 import router as _wave1_router
app.include_router(_wave1_router)

# ─── Wave 3 — AI features (Voice-to-Rx, Patient Gist, Lab OCR) ───
from routers.wave3 import router as _wave3_router
app.include_router(_wave3_router)

# ─── Wave 3 finish + Wave 4 Analytics — Rx suggest, Inbox triage,
# Dashboard widgets, Referrers, Outcomes ───
from routers.wave_more import router as _wave_more_router
app.include_router(_wave_more_router)

# ─── Wave 5 + 6 — Compliance (DPDP export, 2-FA, audit search)
...
@app.on_event("startup")
async def _assert_unique_routes() -> None:
    """Fail-fast on duplicate (path, method) registrations.

    Why: a recent regression registered `/api/analytics/dashboard` in
    BOTH `routers/analytics.py` (legacy) and `routers/wave_more.py`
    (new). FastAPI silently picks the first match — the second
    handler was never called, breaking the analytics widgets feature.
    A simple inventory at boot makes this class of bug impossible to
    miss again.

    We log a single coloured warning per collision AND raise a
    RuntimeError so the supervisor restart loop forces the developer
    to resolve the conflict before traffic flows.
    """
    ...
    for r in app.routes:
        ...
        for method in r.methods or set():
            if method in ("HEAD", "OPTIONS"):
                continue
            seen[(method, r.path)].append(r.endpoint.__module__ + "." + r.endpoint.__name__)

    collisions = {k: v for k, v in seen.items() if len(v) > 1}
    if collisions:
        # Print all collisions before raising so the developer sees
        # the full list, not just the first.
        print("\033[31m[startup] DUPLICATE ROUTE REGISTRATIONS DETECTED:\033[0m")
        for (method, path), endpoints in collisions.items():
            print(f"  {method} {path}")
            for ep in endpoints:
                print(f"    \u2192 {ep}")
        # In production-style envs (prod / staging) we raise; in dev
        # we still raise so the developer fixes it locally.
        raise RuntimeError(
            f"Duplicate FastAPI route registrations: {sorted(collisions.keys())}. "
            "Rename one of the handlers (see logs above)."
        )
    print(f"[startup] route inventory ok \u2014 {len(seen)} unique (method,path) entries.")
