...
export default function PrescriptionDetail() {
  ...
  // Editorial / delete capability — only owner-tier and prescribers see
  // Edit/Delete. Patients have a strictly Print · PDF · Share toolbar
  // (no edit, no delete) per Dr. Joshi spec 2026-05-21.
  const STAFF_RX_ROLES = new Set(['super_owner', 'primary_owner', 'partner', 'owner', 'doctor']);
  const canEditRx = !!user && STAFF_RX_ROLES.has(user.role as string);
  const isOwner = ['super_owner', 'primary_owner', 'partner', 'owner'].includes((user?.role as string) || '');
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: __darkBg }}>
      <View style={styles.topBar}>
        <TouchableOpacity onPress={() => (router.canGoBack() ? router.back() : router.replace('/' as any))} style={styles.backBtn} testID="rx-detail-back">
          <Ionicons name="arrow-back" size={22} color={COLORS.textPrimary} />
        </TouchableOpacity>
        <Text style={styles.title} numberOfLines={1}>Prescription</Text>
        {canEditRx ? (
          <View style={{ flexDirection: 'row', gap: 4 }}>
            {/* Admit Patient — pre-fills the IPD admission form with this Rx's patient details. */}
            <TouchableOpacity
              onPress={() => {
                const p = new URLSearchParams();
                p.set('admit', '1');
                if (rx.patient_name) p.set('patient_name', rx.patient_name);
                if (rx.patient_phone) p.set('patient_phone', rx.patient_phone);
                if (rx.patient_age) p.set('patient_age', String(rx.patient_age));
                if (rx.patient_gender) p.set('patient_gender', rx.patient_gender);
                if (rx.registration_no) p.set('registration_no', rx.registration_no);
                if ((rx as any).patient_address) p.set('address', String((rx as any).patient_address));
                if ((rx as any).patient_email) p.set('patient_email', String((rx as any).patient_email));
                if (rx.diagnosis) p.set('diagnosis', rx.diagnosis);
                if (rx.chief_complaints) p.set('presenting_complaints', rx.chief_complaints);
                if (rx.investigation_findings) p.set('investigations_summary', rx.investigation_findings);
                if (rx.prescription_id) p.set('from_rx', rx.prescription_id);
                router.push(`/ipd?${p.toString()}` as any);
              }}
              style={styles.headerActionBtn}
              hitSlop={10}
              accessibilityLabel="Admit Patient"
              testID="rx-header-admit"
            >
            ...
            {rx.patient_phone ? (
              <TouchableOpacity
                onPress={() => {
                  ...
                  if (rx.patient_name) p.set('patient_name', rx.patient_name);
                  if (rx.patient_age) p.set('patient_age', String(rx.patient_age));
                  if (rx.patient_gender) p.set('patient_sex', rx.patient_gender);
                  if (rx.diagnosis) p.set('diagnosis', rx.diagnosis);
                  p.set('prescription_id', rx.prescription_id || '');
                  router.push(`/ot-calendar/schedule?${p.toString()}` as any);
                }}
                style={styles.headerActionBtn}
                hitSlop={10}
                accessibilityLabel="Schedule OT"
                testID="rx-header-schedule-ot"
              >
