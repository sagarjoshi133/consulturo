...
import GoogleReviewBanner from '../src/google-review-banner';
import { PhrMonthlySummary, IpssSparkline } from '../src/phr-monthly-summary';
...
function MyRecordsImpl() {
  ...
  const [phr, setPhr] = useState<any | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [{ data }, { data: phrData }] = await Promise.all([
        api.get('/records/me'),
        api.get('/patients/me/phr').catch(() => ({ data: null })),
      ]);
      setData(data);
      setPhr(phrData);
      // Cache the PHR for offline access (best-effort).
      try {
        if (phrData) await AsyncStorage.setItem('@consulturo/phr-cache', JSON.stringify({ at: Date.now(), payload: phrData }));
      } catch { /* ignore */ }
    } catch {
      setData(null);
      // Try to restore from cache when offline.
      try {
        const cached = await AsyncStorage.getItem('@consulturo/phr-cache');
        if (cached) {
          const parsed = JSON.parse(cached);
          setPhr(parsed.payload);
        }
      } catch { /* ignore */ }
    } finally {
      setLoading(false);
  ...
  // (router.canGoBack() ? router.back() : router.replace('/' as any)) silently does nothing. Fall back to the home tab.
  ...
  const vitals = [
    {
      ...
      unit: 'mL',
      hint: latestProstate
        ? `${latestProstate.source || 'USG'} · ${latestProstate.measured_on ? (displayDate(latestProstate.measured_on) || '—') : '—'}`
  ...
  return (
    <View style={{ flex: 1, backgroundColor: __darkBg }}>
      ...
      >
        ...
        >
          ...
          {tab === 'overview' && (
            <>
              <GoogleReviewBanner />
              <AppErrorBoundary panelMode label="Monthly summary">
                <PhrMonthlySummary />
              </AppErrorBoundary>
              {/* PHR offline + export actions */}
              <View style={styles.phrActions}>
                <TouchableOpacity
                  style={styles.phrBtn}
                  onPress={async () => {
                    setBusy(true);
                    try {
                      const { data: phrData } = await api.get('/patients/me/phr');
                      await AsyncStorage.setItem('@consulturo/phr-cache', JSON.stringify({ at: Date.now(), payload: phrData }));
                      setPhr(phrData);
                      Alert.alert('Saved offline', `Your records are now available offline (${phrData.timeline?.length || 0} events).`);
                    } catch (e: any) {
                      Alert.alert('Save failed', e?.response?.data?.detail || 'Could not download.');
                    } finally { setBusy(false); }
                  }}
                  disabled={busy}
                  testID="phr-download-offline"
                >
                  <Ionicons name="cloud-download" size={16} color={COLORS.primary} />
                  <Text style={styles.phrBtnText}>Download for offline</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.phrBtn, { backgroundColor: COLORS.primary, borderColor: COLORS.primary }]}
                  onPress={async () => {
                    setBusy(true);
                    try {
                      const { data: tpl } = await api.get('/patients/me/phr/export.html');
                      const { data: rendered } = await api.post('/render/html', { html: tpl.html, filename: tpl.filename });
                      // rendered.url is a data: or http URL — open share dialog
                      const url = rendered?.url || rendered?.pdf_url;
                      if (url && Platform.OS !== 'web') {
                        const dest = (FileSystem.cacheDirectory || '') + (tpl.filename || 'phr.pdf');
                        ...
                        a.href = url; a.download = tpl.filename || 'phr.pdf'; a.click();
                      } else {
                        Alert.alert('Export failed', 'No URL returned by render service.');
                      }
                    } catch (e: any) {
                      Alert.alert('Export failed', e?.response?.data?.detail || 'Could not export.');
                    } finally { setBusy(false); }
                  }}
                  disabled={busy}
                  testID="phr-export-pdf"
                >
                  <Ionicons name="document-text" size={16} color="#fff" />
                  <Text style={[styles.phrBtnText, { color: '#fff' }]}>Export PDF</Text>
                </TouchableOpacity>
              </View>
          ...
          {tab === 'scores' && (
            <>
              {ipss.length === 0 ? (
                ...
                <>
                  {/* IPSS trend sparkline at the top */}
                  <View style={[{ marginBottom: 12, padding: 12, backgroundColor: '#fff', borderRadius: RADIUS.card, borderWidth: 1, borderColor: COLORS.border }, d.surface]}>
                    <Text style={[{ ...FONTS.bodyMedium, color: COLORS.textPrimary, marginBottom: 8 }, d.textP]}>IPSS trend</Text>
                    <AppErrorBoundary panelMode label="IPSS trend">
                      <IpssSparkline points={ipss as any} width={300} height={70} />
                    </AppErrorBoundary>
                  </View>
                  {ipss.map((s) => (
                    <TouchableOpacity
...
function StatCard({ label, value, icon, color }: any) {
  const d = useDarkOverrides();
  return (
    <View style={[styles.statCard, { borderLeftColor: color }, d.surface]}>
      <Ionicons name={icon} size={18} color={color} />
      <Text style={[styles.statVal, { color }]}>{value}</Text>
      <Text style={[styles.statLbl, d.textS]}>{label}</Text>
    </View>
  );
}
  ...
  return (
    <View style={[styles.card, d.surface]}>
      <View style={styles.cardHead}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.cardTitle, d.textP]}>{dateStr}</Text>
          <Text style={[styles.cardMeta, d.textS]}>{b.booking_time ? display12h(b.booking_time) : ''} · {(b.mode === 'online' || b.mode === 'video') ? 'Online (WhatsApp)' : 'In-person'}</Text>
        </View>
        <View style={[styles.statusPill, { backgroundColor: statusColor + '22' }]}>
          <Text style={[styles.statusText, { color: statusColor }]}>{b.status}</Text>
        </View>
      </View>
      {b.reason ? <Text style={[styles.cardBody, d.textP]}>{b.reason}</Text> : null}
    </View>
  );
}
...
function PrescriptionCard({ rx }: { rx: any }) {
  ...
  return (
    <View style={[styles.card, d.surface]}>
      <View style={styles.cardHead}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.cardTitle, d.textP]}>{rx.diagnosis || 'Prescription'}</Text>
          <Text style={[styles.cardMeta, d.textS]}>{dateStr}</Text>
        </View>
        <View style={[styles.statusPill, { backgroundColor: COLORS.primary + '18' }]}>
          <Ionicons name="document-text" size={12} color={COLORS.primary} />
        </View>
      </View>
      {rx.chief_complaints ? <Text style={[styles.cardBody, d.textP]}>{rx.chief_complaints}</Text> : null}
      <Text style={[styles.cardFootMeta, d.textS]}>
...
function SurgeryCard({ s }: { s: any }) {
  ...
  return (
    <View style={[styles.card, d.surface]}>
      <View style={styles.cardHead}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.cardTitle, d.textP]}>{s.surgery_name}</Text>
          <Text style={[styles.cardMeta, d.textS]}>{dateStr}{s.hospital ? ` · ${s.hospital}` : ''}</Text>
        </View>
        <View style={[styles.statusPill, { backgroundColor: COLORS.accent + '18' }]}>
          <Ionicons name="medkit" size={12} color={COLORS.accent} />
        </View>
      </View>
      {s.notes ? <Text style={[styles.cardBody, d.textP]}>{s.notes}</Text> : null}
    </View>
  );
}
...
function ScoreCard({ s }: { s: any }) {
  ...
  return (
    <View style={[styles.card, d.surface]}>
      <View style={styles.cardHead}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.cardTitle, d.textP]}>IPSS Score · {score}/35</Text>
          <Text style={[styles.cardMeta, d.textS]}>
...
const styles = StyleSheet.create({
  ...
  phrActions: { flexDirection: 'row', gap: 8, marginBottom: 14 },
  phrBtn: {
    flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
    paddingVertical: 10, paddingHorizontal: 12, borderRadius: RADIUS.button,
    borderWidth: 1, borderColor: COLORS.primary, backgroundColor: '#fff',
  },
  phrBtnText: { ...FONTS.bodyMedium, color: COLORS.primary, fontSize: 13 },

  timeline: { paddingLeft: 4, paddingRight: 4 },
  timelineRow: { flexDirection: 'row', gap: 10, marginBottom: 4 },
  timelineCol1: { width: 14, alignItems: 'center' },
