...
export default function Login() {
  ...
  // `returnTo` deep-link: when login is triggered from a gated screen
  // (e.g. /pay), the caller passes the path the user came from so we
  // can route them back after a successful sign-in instead of always
  // sending them to /(tabs).
  const returnToParam = useLocalSearchParams<{ returnTo?: string }>().returnTo;
  const safeReturnTo = React.useMemo(() => {
    const raw = (returnToParam || '').toString();
    // Only honour same-origin internal paths to prevent open-redirect.
    if (raw && raw.startsWith('/') && !raw.startsWith('//')) return raw;
    return null;
  }, [returnToParam]);
  const routeAfterAuth = () => {
    if (safeReturnTo) {
      try { router.replace(safeReturnTo as any); return; }
      catch { /* fall through */ }
    }
    router.replace('/(tabs)');
  };
  ...
  return (
    <LinearGradient colors={COLORS.heroGradient} style={{ flex: 1 }}>
      ...
      <PhoneAuthModal
        ...
        onSuccess={onAlternativeSuccess}
        onSwitchToEmail={() => {
          // Bring focus to the email field for the email-OTP flow.
          // We just close the phone modal; the email input is
          // already on screen so the user can tap into it.
          setShowPhone(false);
        }}
      />
    </LinearGradient>
