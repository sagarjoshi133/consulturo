...
export default function PermissionManager() {
  ...
  const [counts, setCounts] = useState<{
    ...
    pdb_total: number;
    pdb_allowed: number;
  }>({ msg_total: 0, msg_allowed: 0, msg_revoked: 0, team_count: 0, pending_bookings: 0, pdb_total: 0, pdb_allowed: 0 });

  const load = useCallback(async () => {
    if (!isOwner) { setLoading(false); return; }
    try {
      const [perms, team, analytics, pdb] = await Promise.all([
        ...
        api.get('/admin/patient-db-permissions').catch(() => ({ data: null })),
      ]);
      ...
      const pdbItems = (pdb as any)?.data?.items || [];
      const pdb_allowed = pdbItems.filter((r: any) => r.can_access_patient_db).length;
      setCounts({
  ...
  const sections: Section[] = [
    ...
    },
    {
      ...
      desc: 'Grant team members read access to the full patient directory, search and consultation history. Export remains owner-only.',
      summary: counts.pdb_total > 0
        ? `${counts.pdb_allowed} of ${counts.pdb_total} team member${counts.pdb_total === 1 ? '' : 's'} allowed`
        : 'No staff members yet',
      onPress: () => router.push('/patient-db-permissions' as any),
    },
    {
      key: 'patient_access',
      icon: 'medical',
    ...
    },
    {
      ...
      desc: 'Scan for and merge users that share an email address (OAuth duplicates, legacy seeds).',
      onPress: () => router.push('/admin/dup-merge' as any),
    },
  ];
