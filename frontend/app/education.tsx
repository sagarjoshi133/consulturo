  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: __darkBg }}>
      ...
      >
        ...
        {/* Patient Care Guides CTA — entry point to /guides */}
        <TouchableOpacity
          onPress={() => router.push('/guides' as any)}
          activeOpacity={0.85}
          style={styles.guidesCta}
          testID="education-guides-cta"
        >
          <View style={styles.guidesCtaIcon}>
            <Ionicons name="medical" size={22} color="#fff" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.guidesCtaTitle}>
              {lang === 'hi' ? 'मरीज़ देखभाल गाइड' : lang === 'gu' ? 'દર્દી સંભાળ માર્ગદર્શિકા' : 'Patient Care Guides'}
            </Text>
            <Text style={styles.guidesCtaSub} numberOfLines={2}>
              {lang === 'hi' ? '33 सर्जरी + 30 स्थितियों के लिए आहार — रिकवरी, क्या करें / क्या न करें' : lang === 'gu' ? '33 સર્જરી + 30 સ્થિતિઓ માટે આહાર — રિકવરી, શું કરવું / શું ન કરવું' : '33 surgeries + 30 condition-based diet plans · recovery · do\'s & don\'ts'}
            </Text>
          </View>
            ...
            {filtered.map((it) => (
              ...
              >
                ...
                <View style={styles.cardBody}>
                  <Text style={[styles.cardTitle, d.textP]} numberOfLines={2}>{it.title}</Text>
                  <Text style={[styles.cardSum, d.textS]} numberOfLines={2}>{it.summary}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  guidesCtaIcon: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.22)', alignItems: 'center', justifyContent: 'center' },
  guidesCtaTitle: { ...FONTS.bodyMedium, color: '#fff', fontSize: 14 },
  guidesCtaSub: { color: 'rgba(255,255,255,0.9)', fontSize: 11.5, marginTop: 2, lineHeight: 16 },
  backBtn: {
    width: 40,
    height: 40,
    borderRadius: 20,
