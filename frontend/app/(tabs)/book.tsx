...
export default function Book() {
  ...
  const navParams = useLocalSearchParams<{ ref?: string }>();
  const { user } = useAuth();
  // Refer-a-Patient — pick up `?ref=<CODE>` from URL, fall back to a
  // cached code in AsyncStorage (so the attribution survives a refresh
  // after the patient landed on /c/<slug>?ref=). The lookup result
  // drives a soft "You were invited by Asha — welcome!" banner.
  const [refCode, setRefCode] = useState<string>('');
  const [refReferrerName, setRefReferrerName] = useState<string>('');
  useEffect(() => {
    (async () => {
      let code = (navParams?.ref || '').toString().trim().toUpperCase();
      if (!code) {
        try {
          const cached = await AsyncStorage.getItem('referral.code');
      ...
      setRefCode(code);
      try {
        const r = await api.get(`/referrals/lookup/${encodeURIComponent(code)}`);
        if (r.data?.valid) {
          setRefReferrerName(r.data.referrer_first_name || '');
          try { await AsyncStorage.setItem('referral.code', code); } catch {}
        } else {
          setRefCode('');
        }
      } catch {
  ...
  // Staff-side affordance — when an owner / staff member books on
  // behalf of a patient (phone call), surface the "Hold payment"
  // toggle so the booking can be marked as pending until the patient
  // settles at the desk. Patients booking themselves never see this.
  const isStaffActor = useMemo(() => {
    if (!user) return false;
    const r = ((user as any).role || '').toLowerCase();
    return ['super_owner', 'primary_owner', 'owner', 'partner', 'doctor', 'assistant', 'receptionist', 'staff'].includes(r);
  }, [user]);
  ...
  // Defaults to in-app video (now that we ship 100ms video). Patient
  // can flip to WhatsApp from the segmented control under the mode row.
  const [onlineChannel, setOnlineChannel] = useState<'in_app' | 'whatsapp'>('in_app');
  // Staff-only: when booking on behalf of a patient (phone call), the
  // fee is collected later at the desk. The toggle flips
  // `pending_offline:true` on creation so the booking lands with
  // payment_status="pending_offline" and the detail card shows a
  // "Mark as paid" button instead of the Razorpay PayButton.
  ...
  const submit = async () => {
    ...
    try {
      const { data } = await api.post('/bookings', {
        ...
        booking_time: slot,
        mode,
        online_channel: mode === 'online' ? onlineChannel : undefined,
        pending_offline: pendingOffline,
        ref_code: refCode || undefined,
        ref_source: refCode ? 'link' : undefined,
      });
      setConfirmed(data);
      haptics.success();
    ...
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: __darkBg }}>
      ...
      >
        <ScrollView
          contentContainerStyle={[
            ...
            isDesktop && {
              ...
              padding: 28,
              paddingBottom: 72,
              backgroundColor: d.isDark ? d.colors.surface : '#fff',
              borderRadius: 16,
              marginTop: 24,
              marginBottom: 24,
              shadowColor: '#0B3142',
              shadowOpacity: d.isDark ? 0 : 0.06,
              shadowRadius: 16,
        ...
        >
          ...
          {/* Owner-curated announcements (booking flow top) */}
          <AnnouncementsBanner audience="patients" placement="booking_flow" style={{ marginBottom: 12 }} />

          {/* Refer-a-Patient — soft welcome banner */}
          {refCode ? (
            <View style={styles.referBanner}>
              <Ionicons name="gift" size={16} color="#9D174D" />
              <Text style={styles.referBannerText}>
                You were invited by <Text style={{ fontWeight: '800' }}>{refReferrerName || 'a friend'}</Text> · Code <Text style={{ fontWeight: '800' }}>{refCode}</Text>
              </Text>
            </View>
          ) : null}
          ...
          <View style={styles.modeRow}>
            {(['in-person', 'online'] as const).map((m) => (
              ...
              >
                <Ionicons
                  name={m === 'in-person' ? 'medical' : 'videocam'}
                  size={16}
          ...
          {/* Online sub-channel picker — shows only when "Online" is
              selected. Lets the patient choose between the new in-app
              video flow (powered by 100ms) and a fallback WhatsApp
              video call. Defaults to In-App. */}
          {mode === 'online' && (
            <View style={styles.channelWrap}>
              <Text style={styles.channelLabel}>
                {t('book.onlineChannelLabel') || 'How do you want the online consultation?'}
              </Text>
              <View style={styles.channelRow}>
                {(['in_app', 'whatsapp'] as const).map((c) => {
                  const active = onlineChannel === c;
                  return (
                    <TouchableOpacity
                      key={c}
                      onPress={() => { haptics.select(); setOnlineChannel(c); }}
                      style={[styles.channelPill, active && styles.channelPillActive, !active && d.surface]}
                      testID={`booking-channel-${c}`}
                      activeOpacity={0.8}
                    >
                      <Ionicons
                        name={c === 'in_app' ? 'phone-portrait' : 'logo-whatsapp'}
                        size={15}
                        color={active ? '#fff' : (c === 'in_app' ? COLORS.primary : '#25D366')}
                      />
                      <Text style={[styles.channelText, active && { color: '#fff' }]}>
                        {c === 'in_app' ? (t('book.onlineAppPill') || 'In-App video') : (t('book.onlineWaPill') || 'WhatsApp video')}
                      </Text>
                    </TouchableOpacity>
                  );
                })}
          ...
          {/* Staff-only: payment-hold toggle for telephonic bookings.
              When the patient confirms a slot by phone but will pay
              cash/UPI at the desk on the day of the visit, staff
              tap this so the booking lands with payment_status
              "pending_offline" and the detail screen surfaces a
              "Mark as paid" button later. */}
          {isStaffActor && (
            <View style={{ marginTop: 18 }}>
              <Text style={styles.channelLabel}>Booking on behalf of patient?</Text>
              <TouchableOpacity
                onPress={() => { haptics.select(); setPendingOffline(v => !v); }}
                style={[styles.channelPill, pendingOffline && styles.channelPillActive, { alignSelf: 'flex-start', marginTop: 4 }]}
                testID="booking-pending-offline"
                activeOpacity={0.85}
              >
                <Ionicons
                  name={pendingOffline ? 'checkbox' : 'square-outline'}
                  size={15}
                  color={pendingOffline ? '#fff' : COLORS.primary}
                />
                <Text style={[styles.channelText, pendingOffline && { color: '#fff' }]}>
                  Hold payment — collect at the desk on visit day
                </Text>
              </TouchableOpacity>
              {pendingOffline ? (
                <Text style={[styles.channelLabel, { marginTop: 6, fontSize: 11, fontWeight: '400' as any }]}>
                  Booking will be marked “Pending payment”. Tap “Mark as paid” on the booking card after the patient pays.
                </Text>
              ) : null}
            </View>
          )}
            ...
            {dates.map((d, idx) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[styles.dateCard, selected && styles.dateCardActive, isToday && !selected && { borderColor: COLORS.primary, borderWidth: 1.5 }, !selected && d.surface]}
...
const styles = StyleSheet.create({
  ...
  referBannerText: { color: '#9D174D', flex: 1, fontSize: 12.5 },
  ...
  channelWrap: { marginTop: 12 },
  channelLabel: { ...FONTS.label, color: COLORS.textSecondary, fontSize: 11, marginBottom: 6 },
  ...
  channelPillActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  channelText: { ...FONTS.bodyMedium, color: COLORS.textPrimary, fontSize: 13 },
