...
import OwnerHome from '../../src/home/owner-home';
import SuperOwnerHome from '../../src/home/super-owner-home';
import StaffHome from '../../src/home/staff-home';
import AssistantBubble from '../../src/assistant-bubble';
import FeaturedReviewsCarousel from '../../src/featured-reviews-carousel';
...
export default function Home() {
  ...
  // Role-aware dispatch — staff/owner/super-owner roles see their
  // operational cockpit instead of the patient-facing home. The
  // existing patient home implementation lives in <PatientHome /> below.
  if (user?.role === 'super_owner') {
    return <SuperOwnerHome onLangPress={cycleLang} langBadge={langBadge} />;
  }
  if (user?.role === 'primary_owner' || user?.role === 'owner') {
    return <OwnerHome onLangPress={cycleLang} langBadge={langBadge} />;
  }
  if (
    user?.role === 'partner' ||
    user?.role === 'doctor' ||
    ...
    return <StaffHome onLangPress={cycleLang} langBadge={langBadge} />;
  }
  return <PatientHome />;
}
...
function PatientHome() {
  ...
  // Phase 5.22 — apply dark-mode root background when the patient
  // has Dark or System=Dark selected in Profile → Appearance. The
  // hero card / quick-action tiles / blog cards keep their own
  // brand colors so the visual identity stays intact; only the
  // gaps between cards flip dark.
  const { colors: darkColors } = useDarkMode();
    ...
    <View style={{ flex: 1, backgroundColor: darkColors.bg }}>
      ...
      >
              ...
              <View style={styles.heroHeader}>
                <View style={styles.heroHeaderLeft}>
                  <Text style={styles.greeting} numberOfLines={1}>
                    {user ? `${t('home.namaste')}, ${(user.name || user.email || '').split(' ')[0] || 'there'}` : t('home.namaste')}
                  </Text>
                  <Text style={styles.brand} numberOfLines={1}>ConsultUro</Text>
                </View>
                <View style={styles.heroActionsCluster}>
                  ...
                  >
                    {user?.picture ? (
                      <Image source={{ uri: user.picture }} style={{ width: 36, height: 36, borderRadius: 18 }} />
                    ) : (
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 12 }}>
          <AnnouncementsBanner audience="patients" placement="patient_home" />
        </View>
        ...
        {/* Section: Featured Patient Reviews — premium gold carousel.
            Sits BELOW Latest Videos per user preference. Auto-hides
            for owner/staff via the component (role check inside). */}
      ...
      {/* Floating AI assistant bubble — patient home only. The owner
          / staff dashboards already surface the assistant elsewhere
          (sidebar entry). */}
...
const styles = StyleSheet.create({
  ...
  // ── Hero header layout (mobile-safe) ────────────────────────────
  // The left column shrinks to fit when the action cluster needs
  // space, preventing the right-most profile button from being
  // pushed off-screen on narrow phones (≤ 360 dp). Action cluster
  // never shrinks (flexShrink: 0) so each tappable circle remains a
  // full 36-dp target.
  heroHeaderLeft: {
    flex: 1,
    minWidth: 0,
