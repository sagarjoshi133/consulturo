...
export default function EgfrCalc() {
  ...
  const { history, loading, saving, saveScore, removeScore } = useToolHistory('egfr', patientCtx);
        ...
        {result && (
          <View style={styles.result}>
            ...
            <SaveScoreButton
              ...
              saving={saving}
            />
            <CopyResultButton text={`eGFR (CKD-EPI 2021): ${result} ml/min/1.73 m² — ${stage(result).label}${patientCtx.patient_name ? ' — Patient: ' + patientCtx.patient_name : ''}`} />
          </View>
        )}
