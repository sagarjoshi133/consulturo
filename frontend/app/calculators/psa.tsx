...
export default function PSACalc() {
  ...
  const { history, loading, saving, saveScore, removeScore } = useToolHistory('psa', patientCtx);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: __darkBg }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        {valid && <CopyResultButton text={`PSA Density: ${density.toFixed(3)} ng/ml/cc — ${high ? t('calc.psa.elevated') : t('calc.psa.acceptable')}${patientCtx.patient_name ? ' — Patient: ' + patientCtx.patient_name : ''}`} />}
