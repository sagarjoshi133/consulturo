...
export default function StonePassCalc() {
  ...
  const { history, loading, saving, saveScore, removeScore } = useToolHistory('stone_risk', patientCtx);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: __darkBg }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        ...
        {result && <CopyResultButton text={`Stone ${result.mm} mm (${location} ureter): ${result.probability}% spontaneous passage — ${recommend(result).label}${patientCtx.patient_name ? ' — Patient: ' + patientCtx.patient_name : ''}`} />}
