...
export default function Videos() {
  ...
  return (
    <SafeAreaView style={[{ flex: 1, backgroundColor: COLORS.bg }, d.screen]}>
      <View style={styles.topBar}>
        <TouchableOpacity onPress={() => (router.canGoBack() ? router.back() : router.replace('/' as any))} style={[styles.backBtn, d.surface]} testID="videos-back">
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60, alignItems: 'center' }}>
        <View style={{ width: '100%', maxWidth: isWebDesktop ? 1100 : 680 }}>
          <Text style={[styles.subtitle, d.textS]}>Educational videos by your urology team</Text>

          <TouchableOpacity
            onPress={() => Linking.openURL(channelUrl)}
            style={[styles.ytBtn, d.surface]}
          ...
          >
            ...
            <Text style={[styles.ytBtnText, d.textP]} numberOfLines={1}>Open YouTube Channel</Text>
            <Ionicons name="arrow-forward" size={14} color={d.colors.textSecondary} />
          </TouchableOpacity>
          ...
          <View style={isWebDesktop ? styles.grid : undefined}>
            {videos.map((v) => (
              <TouchableOpacity
                ...
                activeOpacity={0.86}
                style={[styles.card, isWebDesktop && (isWebWide ? styles.cardDesktop3 : styles.cardDesktop2), d.surface]}
                testID={`video-${v.id}`}
              >
                ...
                <View style={styles.cardBody}>
                  <Text style={[styles.vTitle, d.textP]} numberOfLines={2}>{v.title}</Text>
                  <View style={[styles.metaRow, d.border]}>
                    <Ionicons name="logo-youtube" size={11} color="#E53935" />
                    <Text style={[styles.metaText, d.textS]}>YouTube · Watch →</Text>
                  </View>
                </View>
              </TouchableOpacity>
            ))}
