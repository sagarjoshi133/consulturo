...
import { aiTriage, type TriageResult } from '../src/wave4/api';
...
const TAG_META: Record<string, { label: string; color: string; icon: any }> = {
  all:      { label: 'All',      color: COLORS.textSecondary, icon: 'grid' },
  urgent:   { label: 'Urgent',   color: '#DC2626',            icon: 'alert' },
  question: { label: 'Question', color: '#7C3AED',            icon: 'help-circle' },
  admin:    { label: 'Admin',    color: '#0EA5E9',            icon: 'briefcase' },
  routine:  { label: 'Routine',  color: '#16A34A',            icon: 'checkmark-circle' },
};
...
export default function Inbox() {
  ...
  // ── Wave 3 (P) — Smart Inbox Triage. Staff-only; stores AI tags
  //    keyed by message id so we can colour each card and let the
  //    user filter "Urgent" with a chip.
  const [triageMap, setTriageMap] = useState<Record<string, TriageResult>>({});
  const [triageBusy, setTriageBusy] = useState(false);
  const [triageFilter, setTriageFilter] = useState<'all' | 'urgent' | 'question' | 'admin' | 'routine'>('all');
  ...
  // Wave 3 (P) — Run the AI triage on the visible inbox items. Staff
  // tap "Triage" once; results are cached client-side so re-triage
  // costs only when the visible list changes.
  const runTriage = useCallback(async () => {
    if (!isStaff) return;
    const list = tab === 'sent' ? sent : received;
    const items = list.slice(0, 30).map((m) => ({
      id: m.id,
      text: `${m.title || ''}\n${m.body || ''}`.trim(),
    ...
    setTriageBusy(true);
    try {
      const results = await aiTriage(items);
      const next: Record<string, TriageResult> = { ...triageMap };
  ...
  }, [isStaff, tab, sent, received, triageMap]);
  ...
  const visible = useMemo(() => {
    const list = tab === 'sent' ? sent : received;
    let filtered = list;
    if (isStaff && cpFilter !== 'all') {
      filtered = filtered.filter((i) => {
        const cp = counterpartyOf(i, tab);
        return cpFilter === 'team' ? cp.isStaff : !cp.isStaff;
      });
    }
    if (isStaff && triageFilter !== 'all') {
      filtered = filtered.filter((i) => triageMap[i.id]?.tag === triageFilter);
    }
    return filtered;
  }, [tab, cpFilter, sent, received, isStaff, triageFilter, triageMap]);
  ...
  return (
    <View style={[{ flex: 1, backgroundColor: COLORS.bg }, d.screen]}>
      <Header
      ...
      <View style={styles.mainTabsRow}>
        <TouchableOpacity
          style={[styles.mainTab, tab === 'inbox' && styles.mainTabActive, tab !== 'inbox' && d.surface]}
        ...
        {canSendMsg && (
          <TouchableOpacity
            style={[styles.mainTab, tab === 'sent' && styles.mainTabActive, tab !== 'sent' && d.surface]}
            onPress={() => setTab('sent')}
            activeOpacity={0.78}
      ...
      {/* Triage row — staff only. Tap the wand to AI-classify visible
          messages; chips below let you focus on Urgent/Question/etc. */}
      {isStaff && (
        <View style={styles.triageRow}>
          <TouchableOpacity
            onPress={runTriage}
            style={[styles.triageBtn, triageBusy && { opacity: 0.6 }]}
            disabled={triageBusy}
            activeOpacity={0.78}
            testID="inbox-triage-run"
          >
            {triageBusy ? (
            ...
            <Text style={styles.triageBtnText}>
              {triageBusy ? 'Triaging…' : Object.keys(triageMap).length ? 'Re-triage' : 'AI Triage'}
            </Text>
          </TouchableOpacity>
          {Object.keys(triageMap).length > 0 && (
            ...
            >
              {(['all', 'urgent', 'question', 'admin', 'routine'] as const).map((tag) => {
                const active = triageFilter === tag;
                const meta = TAG_META[tag] || TAG_META.routine;
                return (
                  <TouchableOpacity
                    key={tag}
                    onPress={() => setTriageFilter(tag)}
                    activeOpacity={0.8}
                    style={[
                      styles.tagChip,
                      active && { backgroundColor: meta.color, borderColor: meta.color },
                    ]}
                    testID={`inbox-triage-${tag}`}
                  >
                    <Text style={[styles.tagChipText, active && { color: '#fff' }, !active && { color: meta.color }]}>
                      {meta.label}
                    </Text>
                  </TouchableOpacity>
                );
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.card,
                    d.surface,
                    !n.read && tab === 'inbox' && styles.unreadCard,
                    triage?.tag === 'urgent' && { borderLeftWidth: 4, borderLeftColor: '#DC2626' },
                  ]}
                  activeOpacity={0.78}
                  onPress={() => openItem(n)}
                  testID={`inbox-${n.id}`}
                  ...
                  <View style={styles.cardRow}>
                    ...
                    <View style={{ flex: 1, minWidth: 0 }}>
                      <View style={styles.metaRow}>
                        ...
                        <Text style={[styles.cpName, d.textP]} numberOfLines={1}>
                          {tab === 'sent' ? 'To · ' : 'From · '}
                          {cp.name}
                        </Text>
                        {!!cp.role && (
                        ...
                        {triage ? (
                          <View style={[styles.tagBadge, { backgroundColor: (TAG_META[triage.tag]?.color || '#888') + '18' }]}>
                            <Ionicons
                              name={TAG_META[triage.tag]?.icon || 'pricetag'}
                              size={9}
                              color={TAG_META[triage.tag]?.color || '#888'}
                            />
                            <Text style={[styles.tagBadgeText, { color: TAG_META[triage.tag]?.color || '#888' }]}>
                              {TAG_META[triage.tag]?.label?.toUpperCase() || triage.tag.toUpperCase()}
                            </Text>
                          </View>
                        ) : null}
                      </View>
                      <Text style={[styles.title, d.textP]} numberOfLines={2}>{n.title}</Text>
                      {!!n.body && <Text style={[styles.body, d.textS]} numberOfLines={2}>{n.body}</Text>}
                      {/* Attachment hint */}
...
const styles = StyleSheet.create({
  ...
  triageBtnText: { color: '#fff', ...FONTS.bodyMedium, fontSize: 11 },
  tagChip: {
    paddingHorizontal: 8, paddingVertical: 5,
    backgroundColor: '#fff',
    borderRadius: 12, borderWidth: 1, borderColor: COLORS.border,
  },
  tagChipText: { ...FONTS.bodyMedium, fontSize: 11 },
  tagBadge: {
