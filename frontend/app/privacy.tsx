  ...
  return (
    <View style={[{ flex: 1, backgroundColor: COLORS.bg }, d.screen]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <Text style={[styles.intro, d.textP]}>
...
function Bullet({ children }: { children: React.ReactNode }) {
  ...
  return (
    <View style={styles.bulletRow}>
      ...
      <Text style={[styles.bulletText, d.textP]}>{children}</Text>
    </View>
  );
}
...
function B({ children }: { children: React.ReactNode }) {
  const d = useDarkOverrides();
  return <Text style={[{ fontFamily: 'DMSans_700Bold', color: COLORS.textPrimary }, d.textP]}>{children}</Text>;
}
