...
function Section({ icon, title, children, collapsible = false, defaultOpen = true }: any) {
  ...
  if (!collapsible) {
    return (
      <View style={[styles.section, d.surface]}>
        <View style={styles.sectionHead}>
          <Ionicons name={icon} size={18} color={COLORS.primary} />
          <Text style={[styles.sectionTitle, d.textP]}>{title}</Text>
        </View>
        {children}
      </View>
    );
  ...
  return (
    <View style={[styles.section, d.surface]}>
      <TouchableOpacity onPress={toggle} style={styles.sectionHead} activeOpacity={0.78}>
        <Ionicons name={icon} size={18} color={COLORS.primary} />
        <Text style={[styles.sectionTitle, { flex: 1 }, d.textP]}>{title}</Text>
        <Ionicons name={open ? 'chevron-up' : 'chevron-down'} size={18} color={d.colors.textSecondary} />
      </TouchableOpacity>
