...
function buildTimeline(data: any): TimelineItem[] {
  ...
  (data.appointments || []).forEach((b: any) => {
    ...
    items.push({
      ...
      at,
      title,
      subtitle: [(b.mode === 'online' || b.mode === 'video') ? 'Online' : 'In-person', b.booking_time].filter(Boolean).join(' · '),
...
export default function HealthTimeline() {
  ...
  if (authLoading || loading) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: __darkBg, alignItems: 'center', justifyContent: 'center' }}>
...
function TopBar({ router }: { router: any }) {
  return (
    <View style={styles.topBar}>
      <TouchableOpacity onPress={() => (router.canGoBack() ? router.back() : router.replace('/' as any))} style={styles.iconBtn} testID="timeline-back">
