  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: __darkBg }}>
      <View style={styles.topBar}>
        <TouchableOpacity onPress={() => (router.canGoBack() ? router.back() : router.replace('/' as any))} style={[styles.backBtn, d.surface]} testID="blog-back">
          <Ionicons name="arrow-back" size={22} color={d.colors.textPrimary} />
        </TouchableOpacity>
        <Text style={[styles.title, d.textP]}>{t('blog.title')}</Text>
        <View style={{ flex: 1 }} />
        <TouchableOpacity
          onPress={cycleLang}
          ...
          {filtered.map((p) => (
            ...
            >
              ...
              <View style={styles.cardBody}>
                ...
                <Text style={[styles.postTitle, d.textP]} numberOfLines={2}>{p.title}</Text>
                {p.excerpt ? (
                  <Text style={[styles.postExcerpt, d.textS]} numberOfLines={2}>{p.excerpt}</Text>
                ) : null}
                <View style={styles.postMeta}>
                  <Ionicons name="time-outline" size={11} color={d.colors.textSecondary} />
                  <Text style={[styles.postDate, d.textS]}>{displayDate(p.published_at)}</Text>
