...
function SlideWelcome({ width, active }: { width: number; active: boolean }) {
  const { t } = useI18n();
  return (
    // Slide content can exceed the visible height on very short
    // viewports (small phones, foldable inner displays, web previews
    // at zoom > 100%). Wrapping in a vertical ScrollView keeps the
    // centred look when the content fits, and lets the user reach
    // the bottom credential chips when it doesn't.
    <ScrollView
      style={{ width, flex: 1 }}
      contentContainerStyle={[styles.slide, { paddingVertical: 24 }]}
...
const styles = StyleSheet.create({
  ...
  slide: {
    // The slide is used inside a ScrollView's contentContainerStyle now —
    // `flexGrow: 1` keeps the centred look when the content fits the
    // viewport, and lets it grow + become scrollable when the screen is
    // very short.
    flexGrow: 1,
