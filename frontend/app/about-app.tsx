...
export default function AboutAppPage() {
  ...
  return (
    <SafeAreaView edges={['top']} style={[{ flex: 1, backgroundColor: COLORS.bg }, d.screen]}>
      <View style={[styles.header, d.surface]}>
        <TouchableOpacity onPress={() => (router.canGoBack() ? router.back() : router.replace('/' as any))} style={styles.iconBtn}>
          <Ionicons name="arrow-back" size={20} color={d.colors.textPrimary} />
        </TouchableOpacity>
        <Text style={[styles.title, d.textP]}>About ConsultUro</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.body}>
          <Text style={[styles.bodyHead, d.textP]}>{isStaff ? 'For your team' : 'What you can do'}</Text>
          {list.map((h, i) => (
            <View key={i} style={[styles.card, d.surface]}>
              <View style={styles.cardIcon}>
                <Ionicons name={h.icon} size={22} color={COLORS.primary} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.cardTitle, d.textP]}>{h.title}</Text>
                <Text style={[styles.cardBody, d.textS]}>{h.body}</Text>
              </View>
            </View>
          ))}
          ...
          <View style={styles.footerNote}>
            ...
            <Text style={[styles.footerText, d.textS]}>
              {isStaff
