...
export default function AdminBlog() {
  return (
    <PermissionGate require="can_manage_blog" title="Blog Editor">
      <AdminBlogInner />
    </PermissionGate>
  );
}
...
function AdminBlogInner() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.bg }}>
      <View style={styles.topBar}>
        <TouchableOpacity onPress={() => (mode === 'compose' ? (reset(), setMode('list')) : (router.canGoBack() ? router.back() : router.replace('/' as any)))} style={styles.backBtn} testID="admin-blog-back">
