...
export default function NoteEditor() {
  ...
  const confirmDelete = () => {
    if (isNew) { (router.canGoBack() ? router.back() : router.replace('/' as any)); return; }
  ...
  const confirmLeave = () => {
    if (!dirty) { (router.canGoBack() ? router.back() : router.replace('/' as any)); return; }
    const msg = 'You have unsaved changes. Leave anyway?';
    if (Platform.OS === 'web') {
      if (typeof window !== 'undefined' && window.confirm(msg)) (router.canGoBack() ? router.back() : router.replace('/' as any));
    } else {
      Alert.alert('Unsaved changes', msg, [
        { text: 'Stay', style: 'cancel' },
        { text: 'Leave', style: 'destructive', onPress: () => (router.canGoBack() ? router.back() : router.replace('/' as any)) },
      ]);
    }
  };
