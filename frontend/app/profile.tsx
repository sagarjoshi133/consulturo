...
export default function ProfileScreen() {
  ...
  const confirmSignOut = () => {
    const doSignOut = async () => {
      await signOut();
      // Replace history so user can't back-button into the signed-in app.
      try { router.replace('/signed-out' as any); } catch {}
    };
    if (Platform.OS === 'web') {
      if (typeof window !== 'undefined' && window.confirm(t('profile.signOutConfirmMsg') || 'Sign out of ConsultUro?')) doSignOut();
    } else {
      Alert.alert(
    ...
    <View style={{ flex: 1, backgroundColor: COLORS.bg }}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {/* Dark Mode — Phase 5.22 ─────────────────────────────────
              Three-state segmented control: Light / Dark / System.
              Persisted via the DarkModeProvider's AsyncStorage hook
              so the choice survives app restarts and re-installs of
              the OTA bundle. */}
          <View style={[styles.row, styles.divider, { alignItems: 'flex-start', paddingTop: 14 }]}>
            <View style={[styles.iconWrap, { backgroundColor: '#0E7C8B' + '18' }]}>
              <Ionicons name="moon" size={18} color="#0E7C8B" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.rowTitle}>{t('profile.rowAppearance') || 'Appearance'}</Text>
              <Text style={styles.rowSub}>{t('profile.rowAppearanceSub') || 'Light, Dark, or follow your device theme'}</Text>
        ...
        {/* Security & Privacy — 2FA (owner) + DPDP self-export. */}
        <Text style={styles.sectionLabel}>SECURITY &amp; PRIVACY</Text>
        <View style={styles.section}>
          {isOwner && (
            <TouchableOpacity
              style={styles.row}
              onPress={() => router.push('/admin/two-factor' as any)}
              activeOpacity={0.78}
              testID="profile-2fa"
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.rowSub}>TOTP via Google Authenticator</Text>
              </View>
              <Ionicons name="chevron-forward" size={16} color={COLORS.textDisabled} />
            </TouchableOpacity>
          )}
          ...
          <TouchableOpacity
            style={[styles.row, isOwner && styles.divider]}
            onPress={async () => {
              try {
                const { Linking: L } = require('react-native');
                const base = (process.env.EXPO_PUBLIC_BACKEND_URL || '').replace(/\/$/, '');
                // For STAFF accounts, the DPDP endpoint requires an
                // explicit phone (since staff aren't patients). We pass
                // the staff member's own phone — this lets owners
                // export their own profile bundle. For patients, no
                // phone needed; the backend infers it from the session.
                const params = isStaff && (user as any)?.phone
                  ? `?phone=${encodeURIComponent((user as any).phone)}`
                  : '';
                const url = `${base}/api/dpdp/export${params}`;
                if (Platform.OS === 'web') {
                ...
                Alert.alert('Could not start export', e?.message || 'Try again later.');
              }
            }}
            activeOpacity={0.78}
            testID="profile-dpdp-export"
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.rowSub}>JSON bundle (DPDP §11 · Right to access)</Text>
            </View>
            <Ionicons name="download" size={16} color={COLORS.textDisabled} />
          </TouchableOpacity>
        </View>
      ...
      >
        ...
        <Animated.View style={[StyleSheet.absoluteFill, { paddingTop: insets.top + 8, paddingHorizontal: 16, paddingBottom: 14 }, heroAnim]} pointerEvents="box-none">
          <View style={styles.heroBar}>
            <TouchableOpacity onPress={() => (router.canGoBack() ? router.back() : router.replace('/' as any))} style={styles.heroIconBtn} testID="profile-back">
        ...
        <Animated.View pointerEvents="box-none" style={[styles.compactBar, { paddingTop: insets.top + 4 }, compactAnim]}>
          <View style={styles.compactInner}>
            <TouchableOpacity onPress={() => (router.canGoBack() ? router.back() : router.replace('/' as any))} style={styles.heroIconBtn}>
...
const styles = StyleSheet.create({
  ...
  // Dark Mode segmented control (Phase 5.22)
  segWrap: {
    flexDirection: 'row',
    marginTop: 10,
  ...
  segBtnText: { fontSize: 11.5, fontWeight: '600', color: COLORS.textSecondary },
  segBtnTextActive: { color: COLORS.primary, fontWeight: '700' },
});

/** Three-state segmented control: Light / Dark / System. */
function DarkModeSegmentedControl() {
  const { mode, setMode, effective } = useDarkMode();
  const options: Array<{ key: ThemeMode; label: string; icon: keyof typeof Ionicons.glyphMap }> = [
    { key: 'light',  label: 'Light',  icon: 'sunny' },
    { key: 'dark',   label: 'Dark',   icon: 'moon' },
    { key: 'system', label: 'System', icon: 'phone-portrait' },
  ];
  return (
    <View style={styles.segWrap} testID="dark-mode-segmented">
      {options.map((opt) => {
        ...
        return (
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            testID={`dark-mode-${opt.key}`}
            accessibilityRole="button"
            accessibilityState={{ selected: active }}
            accessibilityLabel={`Use ${opt.label.toLowerCase()} theme${opt.key === 'system' ? ` (currently ${effective})` : ''}`}
          >
            <Ionicons name={opt.icon} size={13} color={iconColor} />
