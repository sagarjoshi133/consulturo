...
import { expoFetchAdapter } from './net/expo-fetch-adapter';
import {
  activateFallback,
  getActiveBase,
  isDrClassError,
...
// by the Expo SDK 54 Android networking regression (requests extremely
// slow / hang — see ./net/expo-fetch-adapter.ts and expo/expo#40061).
// We swap axios onto a self-contained adapter backed by `expo/fetch`
// (Expo's native networking) which bypasses the broken path and is fast.
// iOS + Web keep axios's default adapter (they are unaffected).
const androidFetchAdapter =
  Platform.OS === 'android' ? expoFetchAdapter : undefined;
