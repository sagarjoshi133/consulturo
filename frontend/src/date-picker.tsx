...
/* ─────────────────────────────────────────────────────────────────
   ISODateField — same UI as DateField but keeps its `value` and
   `onChange` payload as ISO yyyy-MM-dd. Use this when the consumer
   already stores ISO strings (most backend-bound forms do) so we
   avoid round-trip conversion at every call-site.
───────────────────────────────────────────────────────────────── */
...
export function ISODateField({ value, onChange, ...rest }: ISODateFieldProps) {
  const toUI = (iso: string): string => {
    if (!iso) return '';
    const d = parse(iso, 'yyyy-MM-dd', new Date());
    return isValid(d) ? format(d, UI_DATE_FORMAT) : '';
  };
  const fromUI = (ui: string): string => {
    if (!ui) return '';
    return parseUIDate(ui) || '';
  };
  return (
    <DateField
      {...rest}
      value={toUI(value)}
      onChange={(ui) => onChange(fromUI(ui))}
    />
  );
}
