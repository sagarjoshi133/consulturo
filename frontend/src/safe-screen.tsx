...
export default function SafeScreen({
  ...
  // Decide keyboard avoidance default per platform — both iOS and
  // Android now opt in. Android pairs with `softwareKeyboardLayoutMode:
  // "resize"` in app.json so the screen actually shrinks behind the
  // keyboard rather than the IME painting on top of the form.
  const shouldAvoid = typeof avoidKeyboard === 'boolean' ? avoidKeyboard : true;
    ...
    <>
      {scroll ? (
        <ScrollView
          ...
          // iOS-only: lets the OS auto-scroll the focused TextInput
          // above the keyboard, so users never lose sight of what
          // they're typing. Android handles this via the
          // `softwareKeyboardLayoutMode: "resize"` manifest flag.
