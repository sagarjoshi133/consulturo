...
export function NotificationsHealthPanel() {
  ...
  const runTest = async () => {
    ...
    try {
      ...
      if (r.ok) {
        haptics.success();
        if (r.transport === 'emergent_relay') {
          // Production transport — the relay resolves device tokens
          // upstream, so we report its outcome directly.
          Alert.alert(
            'Test push sent',
            (r.message || 'Sent via the Emergent push relay.') +
