  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 80 }} showsVerticalScrollIndicator={false}>
      ...
      {/* Prescription Letterhead — branding for the Rx PDF. Visible to
          ...
          Gated on `rxOnly` so this block ONLY appears under the
          dedicated "Rx" tab in the consolidated Branding & Settings
          panel — never under the generic "Branding" tab (those were
          duplicates and confused the doctor, fixed Jun-15). */}
      {rxOnly && (!tier.isPartner || s.partner_can_edit_branding !== false) && (
        <>
          ...
          {/* ─── Print Mode (pre-printed offset letterhead) ─────────
              When ON, the Rx PDF SUPPRESSES the entire app-rendered
              header AND footer (logo, clinic strip, QR, signature
              card, promise block, dashed footer, watermark) and
              prints only the dynamic body in plain black ink. Used
              by clinics that feed pre-printed offset letterhead
              pages into the clinic printer — the digital body
              slots into the blank area below the printed header.
              Faster (no full-colour images) AND aligns perfectly. */}
          <View style={[styles.toggleRow, { marginTop: 14 }]}>
            <View style={{ flex: 1 }}>
              <Text style={styles.toggleLabel}>
                Print Mode (pre-printed letterhead)
              </Text>
              <Text style={[styles.help, { marginTop: 2, marginBottom: 0 }]}>
                Suppresses header &amp; footer — only the prescription
                body is printed in plain black ink, perfectly aligned
                to your offset-printed letterhead paper.
              </Text>
            </View>
            <TouchableOpacity
              onPress={() => {
                const v = !s.rx_print_mode;
                set('rx_print_mode', v);
                save({ rx_print_mode: v });
              }}
              style={[styles.toggle, !!s.rx_print_mode && styles.toggleOn]}
              testID="branding-toggle-printmode"
            >
              <View style={[styles.toggleDot, !!s.rx_print_mode && styles.toggleDotOn]} />
            </TouchableOpacity>
          </View>

          {/* When Print Mode is ON, expose the top-offset slider so the
              doctor can dial in the exact gap (mm) where the printed
              letterhead ends and the digital body should begin. */}
          {s.rx_print_mode ? (
            <View
              style={{
                marginTop: 10,
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Text style={{ ...FONTS.bodyMedium, color: COLORS.primary, fontSize: 14 }}>
                  {Number(s.rx_print_top_mm ?? 40)} mm
                </Text>
              </View>
              <Text style={[styles.help, { marginTop: 4 }]}>
                Distance from the top of the page to where the
                prescription body begins. Increase this if your printed
                header is tall; decrease it if it&apos;s short. Most
                offset letterheads need 35–50 mm.
              </Text>
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                {[20, 30, 40, 50, 60, 70].map((mm) => {
                  const active = Number(s.rx_print_top_mm ?? 40) === mm;
                  return (
                    <TouchableOpacity
                      key={mm}
                      onPress={() => {
                        set('rx_print_top_mm', mm);
                        save({ rx_print_top_mm: mm });
                      }}
                      style={{
                        flex: 1,
                        paddingVertical: 8,
                      ...
                      testID={`branding-printmode-top-${mm}`}
                    >
                      <Text style={{
                        ...FONTS.bodyMedium,
                        fontSize: 12,
