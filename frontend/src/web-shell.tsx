...
export function WebShell({ children }: { children: React.ReactNode }) {
  ...
  // Embedded mode — when a screen is loaded inside an iframe (e.g. the
  // tablet two-pane Bookings layout opens /bookings/[id]?embedded=1
  // in an iframe), suppress the desktop sidebar so the iframe shows
  // only the actual content. The flag is set on the URL.
  const isEmbedded =
    typeof window !== 'undefined' &&
    (window.location?.search || '').includes('embedded=1');

  // Mobile-first: pass through unchanged.
  if (!r.isWebDesktop || isAuthScreen || isEmbedded) {
    return <>{children}</>;
  }
...
function DesktopShell({ children }: { children: React.ReactNode }) {
  ...
  // Editorial gate — super_owner always; primary_owner only when
  // super_owner has flipped `can_create_blog: true`. Matches the
  // backend `require_blog_writer` dependency.
  const tier = useTier();
  const canCreateBlog = isSuperOwner || isOwner || !!tier?.canCreateBlog || !!(user as any)?.can_manage_blog;
  // Session 5.9 — per-ability gates for sidebar discovery.
  const _u = (user as any) || {};
  const canBilling   = isOwner || isFullAccess || !!_u.can_manage_billing;
  const canIpd       = isOwner || isFullAccess || !!_u.can_manage_ipd;
  const canAnalytics = isOwner || isFullAccess || !!_u.can_view_analytics;
  const canSettings  = isOwner || isFullAccess || !!_u.can_manage_settings;
  ...
  const items: NavItem[] = [
    ...
    // AI Assistant — surfaced as a top-level sidebar entry so it is
    // discoverable for owner / staff (the floating bubble alone was
    // easy to miss). Patient role also gets it for parity.
    {
      label: lang === 'hi' ? 'AI सहायक' : lang === 'gu' ? 'AI સહાયક' : 'AI Assistant',
      icon: 'sparkles',
      route: '/assistant',
    },
  ];
  ...
  if (isSuperOwner) {
    ...
    items.push({ label: t('more.createBlog') || 'Create Blog Post', icon: 'newspaper', route: '/admin/blog', ownerOnly: true });
    // Platform-wide nuclear reset — super_owner only. End of the
    // platform-admin block so the eye finds it last (lowest-use,
    // highest-blast-radius).
    items.push({ label: 'Platform Reset', icon: 'nuclear', route: '/admin/platform-reset', ownerOnly: true });
  } else if (isStaff) {
  ...
  // Strict order per user spec (Jun-16): Consults · Prescriptions ·
  // Consents · OT Schedule · Surgeries · IPD · Discharge Summary ·
  // Medical Certificate · Inbox · Broadcasts · Notes · Reminders.
  // Inbox is auto-deduped here — it's already pushed once in the
  // Main section above (line ~245), so we DON'T add it again in
  ...
  if (isStaff && !isSuperOwner) {
    ...
    if (canIpd) {
      items.push({ label: 'IPD',                                       icon: 'bed', route: '/ipd', staffOnly: true });
    }
    items.push({ label: 'Discharge Summary',                          icon: 'exit', route: '/discharge-summaries', staffOnly: true });
    items.push({ label: 'Medical Certificate',                        icon: 'ribbon', route: '/medical-certificates', staffOnly: true });
  ...
  // Strict order per user spec (Jun-16):
  //   Team · Analytics · Billing & Receipts · Fees · Permission
  //   Manager · Audit Log · Branding · Announcements · Video ·
  //   Blog Admin · App stability log · Backups.
  // Clinic Link kept as an appendage AFTER Backups so the shortcut
  // isn't lost (user spec doesn't include it but it's needed for
  // sharing the patient-facing clinic URL).
  // Super-owner already has these (separate Analytics + Backups
  // under Platform Administration above) — guard against duplicates.
  if (isStaff && !isSuperOwner) {
    // 1. Team
    if ((isOwner || isFullAccess)) {
      items.push({ label: t('more.team') || 'Team', icon: 'people', route: '/dashboard?tab=team', staffOnly: true, section: SEC_ADMIN });
    }
    // 2. Analytics
    if (canAnalytics) {
      items.push({ label: t('more.analytics') || 'Analytics', icon: 'analytics', route: '/dashboard?tab=analytics', staffOnly: true });
    }
    // 3. Billing & Receipts
    if (canBilling) {
      items.push({ label: 'Billing & Receipts', icon: 'card', route: '/billing', staffOnly: true });
    }
    // 4. Fees
    if (canSettings) {
      items.push({ label: 'Fees', icon: 'cash', route: '/admin/billing-settings', staffOnly: true });
    }
    // 5. Permission Manager
    if (isOwner) {
      items.push({ label: t('more.permissionManager') || 'Permission Manager', icon: 'key', route: '/permission-manager', ownerOnly: true });
      // 6. Audit Log
      items.push({ label: 'Audit Log', icon: 'time', route: '/admin/audit-log', ownerOnly: true });
    }
    // 7. Branding settings
    if (canSettings) {
      items.push({ label: t('more.branding') || 'Branding', icon: 'color-palette', route: '/branding', staffOnly: true });
    }
    // 8. Announcements
    if (isOwner) {
      items.push({ label: 'Announcements', icon: 'megaphone', route: '/admin/announcements', ownerOnly: true });
    }
    // 9. Video settings
    if (canSettings) {
      items.push({ label: 'Video', icon: 'videocam', route: '/admin/video-settings', staffOnly: true });
    }
    // 10. Blog Admin
    if (canCreateBlog) {
    ...
    // 12. Backups — last item per user spec.
    if (isOwner || isFullAccess) {
      items.push({ label: t('more.backups') || 'Backups', icon: 'cloud-upload', route: '/dashboard?tab=backups', staffOnly: true });
    }
    // 13. Factory Reset — super_owner + primary_owner only (per
    //     user spec). Shows up AFTER Backups so the warning page's
    //     "take a backup first" reminder lines up visually with the
    //     adjacent Backups shortcut. Distinct red label is rendered
    //     by the sidebar's normal active-tile renderer (no special
    //     CSS — keeps the sidebar generic).
    if (isSuperOwner || tier?.isPrimaryOwner) {
      items.push({ label: 'Factory Reset', icon: 'nuclear', route: '/admin/factory-reset', staffOnly: true });
    }
    // Appendage — Clinic Link shortcut for primary_owner / partner.
    if (isOwner && clinicLink) {
      items.push({
  ...
  items.push({ label: 'AI Assistant',                              icon: 'sparkles', route: '/assistant' });
            ...
            >
              ...
              {!collapsed && (
                <>
                  ...
                  <TouchableOpacity
                    onPress={async () => { await signOut(); try { router.replace('/signed-out' as any); } catch {} }}
