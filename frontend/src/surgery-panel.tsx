...
export function SurgeriesPanel({ autoOpen = 0 }: { autoOpen?: number } = {}) {
  ...
  // Header validation — required columns must be present (case/space-insensitive).
  // The backend canonicalises these aliases too; we mirror the most common variants
  // so the user is warned BEFORE pressing Import.
  const REQUIRED_FIELDS = [
    { label: 'Name', aliases: ['name', 'patient name', 'patient'] },
    { label: 'Surgery', aliases: ['name of surgery', 'surgery', 'procedure', 'operation', 'surgery name'] },
    { label: 'Date', aliases: ['date of surgery', 'date', 'dos', 'surgery date', 'operation date'] },
  ];
  ...
  const validateHeaders = useCallback((headers: string[]) => {
    ...
    for (const f of REQUIRED_FIELDS) {
      const found = f.aliases.some((a) => normSet.has(normHeader(a)));
      if (!found) missing.push(f.label);
    }
    return missing;
  }, []);
  ...
  const preview = useMemo(() => {
    ...
    try {
      ...
      return { headers, rows, missing, total: rows.length };
    } catch {
      return null;
    }
  }, [csvText, validateHeaders]);

  // Categorise backend errors so the result card can group them with
  // colour & action hints instead of dumping a wall of red text.
  const categorise = (errs: any[]) => {
    const groups: Record<string, { label: string; icon: any; tone: 'warn' | 'err'; hint: string; items: any[] }> = {
      missing: { label: 'Missing required fields', icon: 'alert-circle-outline', tone: 'err', hint: 'Add name, surgery and date — those rows are skipped.', items: [] },
      date: { label: 'Bad date format', icon: 'calendar-outline', tone: 'warn', hint: 'Use DD-MM-YYYY, DD/MM/YYYY, or 3-Mar-2025.', items: [] },
      shape: { label: 'Malformed row', icon: 'document-outline', tone: 'err', hint: 'A line had the wrong number of columns. Re-export from your source.', items: [] },
      db: { label: 'Server / DB errors', icon: 'cloud-offline-outline', tone: 'err', hint: 'Server rejected the row. Retry, or contact support if it persists.', items: [] },
    };
    for (const e of errs) {
      const msg = (e?.error || '').toLowerCase();
      if (msg.includes('missing')) groups.missing.items.push(e);
      else if (msg.includes('date')) groups.date.items.push(e);
      else if (msg.includes('not an object')) groups.shape.items.push(e);
      else groups.db.items.push(e);
    }
    return groups;
  };
  ...
  const pickCsvFile = async () => {
    try {
      ...
      if (!asset?.uri) return;
      // Web: blob via fetch; native: FileSystem.
      let text = '';
      if (Platform.OS === 'web') {
      ...
      Platform.OS === 'web' ? window.alert(msg) : Alert.alert('File error', msg);
    }
  };

  const downloadErrorsCsv = useCallback(async () => {
    if (!importResult?.errors?.length) return;
    const rows = importResult.errors.map((e) => ({
      row_number: (e.row != null ? e.row + 1 : ''),
      error: e.error || '',
    }));
    ...
    const body = rows
      .map(
        (r) =>
          `${r.row_number},"${String(r.error).replace(/"/g, '""')}"`,
      )
      .join('\n');
    const csv = `${header}\n${body}\n`;
    const today = new Date().toISOString().slice(0, 10);
    const filename = `consulturo-import-errors-${today}.csv`;
    try {
      if (Platform.OS === 'web') {
        ...
        if (await Sharing.isAvailableAsync()) {
          await Sharing.shareAsync(path, { mimeType: 'text/csv', dialogTitle: 'Import errors' });
        } else {
          Alert.alert('Saved', `File saved to: ${path}`);
        }
      }
    } catch (e: any) {
      const msg = e?.message || 'Could not export errors';
  ...
  return (
    <>
      ...
      <Modal visible={showImport} animationType="slide" onRequestClose={() => { setShowImport(false); resetImportState(); }}>
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.bg }} edges={['top', 'bottom']}>
          <View style={styles.modalHeader}>
            <TouchableOpacity onPress={() => { setShowImport(false); resetImportState(); }} testID="sx-import-close">
              <Ionicons name="close" size={24} color={COLORS.textPrimary} />
            </TouchableOpacity>
          ...
          <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.importSourceCard}>
              <View style={{ flexDirection: 'row', gap: 10 }}>
                <TouchableOpacity
                  ...
                  testID="sx-import-pick-file"
                  activeOpacity={0.8}
                >
                  <Ionicons name="document-attach-outline" size={18} color={COLORS.primary} />
                  <Text style={styles.importSourceBtnText}>Pick CSV file</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() =>
                    setCsvText(
                  ...
                  style={styles.importSourceBtnGhost}
                >
                  <Ionicons name="sparkles-outline" size={16} color={COLORS.textSecondary} />
                  <Text style={styles.importSourceBtnGhostText}>Load sample</Text>
                </TouchableOpacity>
                {csvText.trim() ? (
                  <TouchableOpacity
                    onPress={resetImportState}
                    style={styles.importSourceBtnGhost}
                    testID="sx-import-clear"
                  >
                    <Ionicons name="refresh-outline" size={16} color={COLORS.textSecondary} />
                    <Text style={styles.importSourceBtnGhostText}>Clear</Text>
                  </TouchableOpacity>
                ) : null}
              </View>
              {csvFileName ? (
                <View style={styles.importFilePill}>
                  <Ionicons name="document-text" size={14} color={COLORS.primary} />
                  <Text style={styles.importFilePillText} numberOfLines={1}>{csvFileName}</Text>
                </View>
              ) : null}
            </View>

            <Text style={{ ...FONTS.body, color: COLORS.textSecondary, marginBottom: 8, lineHeight: 20 }}>
              Export your Uro Logbook to CSV and paste below — or pick the CSV file directly. Required columns:{' '}
              <Text style={{ fontWeight: '700', color: COLORS.textPrimary }}>Name, Name of Surgery, Date of Surgery</Text>.
              Optional: Mobile, Age, Sex, IP No., Address, Category, Consultation Date, Referred By, Diagnosis, Imaging,
              Department, Date of Admission, Hospital, Operative Findings, Post-op Investigations, Date of Discharge, Follow up, Notes.
            </Text>
            <Text style={{ ...FONTS.body, color: COLORS.textSecondary, marginBottom: 12, fontSize: 12 }}>
              Dates accepted as DD-MM-YYYY, DD/MM/YYYY, YYYY-MM-DD, or 3-Mar-2025.
            </Text>

            <TextInput
              value={csvText}
              onChangeText={(t) => { setCsvText(t); setCsvFileName(null); setImportResult(null); }}
            ...
            {/* Pre-flight panel — shows BEFORE the user taps Import */}
            {preview && (
              <View style={styles.preflightCard}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <Ionicons
                    name={preview.missing.length === 0 ? 'checkmark-circle' : 'warning'}
                    size={18}
                    color={preview.missing.length === 0 ? COLORS.success : '#D97706'}
                  />
                  <Text style={[styles.preflightTitle, { color: preview.missing.length === 0 ? COLORS.success : '#D97706' }]}>
                    {preview.missing.length === 0
                      ? `Ready to import — ${preview.total} row${preview.total === 1 ? '' : 's'} detected`
                      : `Missing required column${preview.missing.length === 1 ? '' : 's'}`}
                  </Text>
                </View>
                <Text style={styles.preflightSub}>
                  Detected {preview.headers.length} column{preview.headers.length === 1 ? '' : 's'}, {preview.total} row{preview.total === 1 ? '' : 's'}.
                </Text>
                {preview.missing.length > 0 ? (
                  <View style={styles.preflightBadgeRow}>
                    {preview.missing.map((m) => (
                      <View key={m} style={styles.preflightBadgeWarn}>
                        <Ionicons name="alert-circle" size={11} color="#92400E" />
                        <Text style={styles.preflightBadgeWarnText}>{m}</Text>
                      </View>
                    ))}
                  </View>
                ) : null}
                {/* Header chips */}
                <View style={styles.preflightBadgeRow}>
                  {preview.headers.slice(0, 14).map((h, i) => (
                    <View key={`h-${i}`} style={styles.preflightHeaderChip}>
                      <Text style={styles.preflightHeaderChipText} numberOfLines={1}>{h}</Text>
                    </View>
                  ))}
                  {preview.headers.length > 14 ? (
                    <View style={styles.preflightHeaderChip}>
                      <Text style={styles.preflightHeaderChipText}>+{preview.headers.length - 14} more</Text>
                    </View>
                  ) : null}
                </View>
              </View>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginTop: 14 }}>
              <PrimaryButton
                title={importing ? 'Importing…' : `Import${preview?.total ? ` ${preview.total} rows` : ' Rows'}`}
                onPress={runImport}
            ...
            {/* Result panel */}
            {importResult && (
              <View style={styles.importResultWrap}>
                {/* Summary header */}
                <View style={styles.importSummaryRow}>
                  <View style={[styles.importSummaryTile, { backgroundColor: '#ECFDF5', borderColor: '#A7F3D0' }]}>
                    <Ionicons name="checkmark-circle" size={20} color={COLORS.success} />
                    <Text style={[styles.importSummaryNum, { color: COLORS.success }]}>{importResult.inserted}</Text>
                    <Text style={styles.importSummaryLbl}>Imported</Text>
                  </View>
                  <View style={[styles.importSummaryTile, {
                    backgroundColor: importResult.errors?.length ? '#FEF2F2' : '#F1F5F9',
                    borderColor: importResult.errors?.length ? '#FECACA' : COLORS.border,
                  }]}>
                    <Ionicons
                      name={importResult.errors?.length ? 'alert-circle' : 'remove-circle-outline'}
                      size={20}
                      color={importResult.errors?.length ? COLORS.accent : COLORS.textSecondary}
                    />
                    <Text style={[styles.importSummaryNum, {
                      color: importResult.errors?.length ? COLORS.accent : COLORS.textSecondary,
                    }]}>{importResult.errors?.length || 0}</Text>
                    <Text style={styles.importSummaryLbl}>Skipped</Text>
                  </View>
                  <View style={[styles.importSummaryTile, { backgroundColor: '#EFF6FF', borderColor: '#BFDBFE' }]}>
                    <Ionicons name="documents-outline" size={20} color="#1D4ED8" />
                    <Text style={[styles.importSummaryNum, { color: '#1D4ED8' }]}>{importResult.total}</Text>
                    <Text style={styles.importSummaryLbl}>Total rows</Text>
                  </View>
                </View>

                {/* Success state */}
                {importResult.inserted > 0 && (!importResult.errors || importResult.errors.length === 0) ? (
                  <View style={styles.importSuccessBanner}>
                    <Ionicons name="ribbon" size={16} color={COLORS.success} />
                    <Text style={styles.importSuccessBannerText}>
                      All {importResult.inserted} rows imported successfully. They&apos;re now in your logbook.
                    </Text>
                  </View>
                ) : null}
                ...
                {importResult.errors && importResult.errors.length > 0 ? (
                  <>
                    {Object.entries(categorise(importResult.errors)).map(([key, grp]) => {
                      if (grp.items.length === 0) return null;
                      const isErr = grp.tone === 'err';
                      return (
                        <View
                          key={key}
                          style={[
                        ...
                        >
                          <View style={styles.errGroupHead}>
                            <Ionicons
                              name={grp.icon}
                              size={16}
                              color={isErr ? COLORS.accent : '#D97706'}
                            />
                            <Text style={[styles.errGroupTitle, { color: isErr ? COLORS.accent : '#92400E' }]}>
                              {grp.label} · {grp.items.length}
                            </Text>
                          </View>
                          <Text style={styles.errGroupHint}>{grp.hint}</Text>
                          {grp.items.slice(0, 6).map((e: any, i: number) => (
                            <View key={i} style={styles.errRow}>
                              <View style={styles.errRowBadge}>
                                <Text style={styles.errRowBadgeText}>#{(e.row != null ? e.row + 1 : '?')}</Text>
                              </View>
                              <Text style={styles.errRowText} numberOfLines={3}>{e.error}</Text>
                            </View>
                          ))}
                          {grp.items.length > 6 ? (
                            <Text style={styles.errGroupMore}>+{grp.items.length - 6} more — download CSV to see all</Text>
                          ) : null}
                        </View>
                      );
                    })}
                    <TouchableOpacity
                      onPress={downloadErrorsCsv}
                      style={styles.errDownloadBtn}
                      testID="sx-import-download-errors"
                    >
                      <Ionicons name="download-outline" size={14} color={COLORS.primary} />
                      <Text style={styles.errDownloadBtnText}>Download errors CSV</Text>
                    </TouchableOpacity>
                  </>
                ) : null}
              </View>
...
const styles = StyleSheet.create({
  ...
  // ── Enhanced CSV import UI (Phase 7) ─────────────────────────────
  importSourceCard: {
    backgroundColor: '#fff',
    padding: 12,
  ...
  importSourceBtnText: { ...FONTS.bodyMedium, color: COLORS.primary, fontSize: 13 },
  importSourceBtnGhost: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
    backgroundColor: '#fff',
  ...
  importSourceBtnGhostText: { ...FONTS.bodyMedium, color: COLORS.textSecondary, fontSize: 12 },
  importFilePill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  ...
  importFilePillText: { ...FONTS.bodyMedium, color: COLORS.primary, fontSize: 12, maxWidth: 260 },
  ...
  preflightTitle: { ...FONTS.bodyMedium, fontSize: 14, flexShrink: 1 },
  preflightSub: { ...FONTS.body, color: COLORS.textSecondary, fontSize: 12 },
  preflightBadgeRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 8 },
  preflightBadgeWarn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  ...
  preflightBadgeWarnText: { ...FONTS.bodyMedium, color: '#92400E', fontSize: 11 },
  preflightHeaderChip: {
  ...
  preflightHeaderChipText: { ...FONTS.body, color: COLORS.textSecondary, fontSize: 11 },

  importResultWrap: { marginTop: 16, gap: 10 },
  importSummaryRow: { flexDirection: 'row', gap: 8 },
  importSummaryTile: {
    flex: 1,
    alignItems: 'center',
    paddingVertical: 12,
  ...
  importSummaryNum: { ...FONTS.h3, fontSize: 22 },
  importSummaryLbl: { ...FONTS.body, color: COLORS.textSecondary, fontSize: 11 },
  importSuccessBanner: {
    flexDirection: 'row',
    gap: 8,
    alignItems: 'center',
  ...
  importSuccessBannerText: { ...FONTS.body, color: '#065F46', fontSize: 13, flex: 1, lineHeight: 18 },
  ...
  errGroupHead: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  errGroupTitle: { ...FONTS.bodyMedium, fontSize: 13 },
  errGroupHint: { ...FONTS.body, color: COLORS.textSecondary, fontSize: 11.5, marginTop: 2, marginBottom: 6, lineHeight: 16 },
  errRow: {
    flexDirection: 'row',
  ...
  errRowBadgeText: { ...FONTS.bodyMedium, fontSize: 11, color: COLORS.textPrimary },
  errRowText: { ...FONTS.body, fontSize: 12, color: COLORS.textPrimary, flex: 1, lineHeight: 17 },
  errGroupMore: { ...FONTS.body, fontSize: 11, color: COLORS.textSecondary, marginTop: 4 },
  errDownloadBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  ...
  errDownloadBtnText: { ...FONTS.bodyMedium, color: COLORS.primary, fontSize: 12 },
