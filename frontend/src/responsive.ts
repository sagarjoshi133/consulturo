...
/**
 * Hook — returns whether the current viewport should render a master-
 * detail two-pane layout (e.g. Bookings list on the left, booking
 * detail on the right).
 *
 * Activates when:
 *   • On web AND in landscape orientation
 *   • Width ≥ 1024 px (desktop) OR iPad-class landscape (900+ wide AND
 *     600+ tall)
 *   • `forcedView` is NOT 'mobile' (so the user's explicit phone
 *     preference always wins)
 *
 * Returns the suggested list pane width too so callers can wire flex
 * proportions deterministically.
 */
export type TwoPaneInfo = {
  twoPane: boolean;
  listWidth: number;
  detailFlex: number;
};

export function useTwoPaneLayout(): TwoPaneInfo {
  const r = useResponsive();
  if (Platform.OS !== 'web') return { twoPane: false, listWidth: 0, detailFlex: 1 };
  if (r.forcedView === 'mobile') return { twoPane: false, listWidth: 0, detailFlex: 1 };
  const landscape = r.width > r.height;
  // iPad-class landscape: 900–1023 wide, in landscape. Desktop: 1024+.
  const ok = landscape && r.width >= 900 && r.height >= 600;
  if (!ok) return { twoPane: false, listWidth: 0, detailFlex: 1 };
  // Pick a list pane width proportional to total width but capped.
  // 380–460 looks right for booking-card list density.
  const listWidth = Math.max(360, Math.min(460, Math.round(r.width * 0.36)));
  return { twoPane: true, listWidth, detailFlex: 1 };
}
