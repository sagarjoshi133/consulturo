export const COLORS = {
  ...
  textDisabled: '#A0B5B8',
  // Backward-compat alias used across the codebase. Mirrors
  // `textDisabled` so the long tail of files referencing this key
  // (audit log, IPD module, refer screen, etc.) never produces an
  // `undefined` color in production where stricter style validation
  // could throw.
  textTertiary: '#A0B5B8',
  border: '#E2ECEC',
  // Soft danger red used for delete / destructive UI.
  danger: '#DC2626',
...
export const RADIUS = {
  ...
  lg: 24,
  pill: 9999,
  // Semantic aliases — used across many screens (IPD, announcements,
  // refer, audit log, etc.). Kept as concrete numbers so the type
  // checker stops failing and so production builds never see an
  // `undefined` borderRadius (which on stricter platforms is rejected).
  card: 16,
  button: 12,
  input: 10,
};

// Bold + regular font weight aliases — referenced by audit-log and
// owner analytics screens. Kept here so consumers don't crash at
// runtime when they read FONTS.bold / FONTS.regular.
