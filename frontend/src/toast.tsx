...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  // ─── CRITICAL: memoise the context value ─────────────────────
  // `value` MUST be stable across renders. If we built it inline
  // every render, every consumer of `useToast()` would receive a
  // brand-new `toast` object after every toast — which silently
  // invalidated downstream `useCallback`s and `useEffect` deps that
  // listed `toast` in their dependency array. That cascaded into a
  // very visible bug on Jun-15: tapping "Generate with AI" in the
  // Discharge tab would succeed → call `toast.success(...)` → the
  // ToastProvider re-rendered → ALL admission-detail / discharge-
  // tab `useEffect([..., toast])` blocks re-fired → the screen
  // "reloaded" and the AI-drafted text was discarded. Memoising
  // here keeps `toast` referentially stable forever.
  // The four methods are wrapped in their own `useCallback` so
  // they too keep stable identities for dependency arrays.
