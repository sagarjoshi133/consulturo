...
type Member = {
  ...
  /** Session 5.9 — fine-grained ability gates. */
  can_manage_billing?: boolean;
  can_manage_ipd?: boolean;
...
export function TeamPanelV2() {
  ...
  const tier = useTier();
  // Only an owner-tier user with blog editorial access can grant the
  // "Can write Blog posts" ability to a team member. We hide the
  // toggle entirely when the current user can't blog themselves —
  // mirrors the backend guard in /api/team/invites and /api/team/{email}.
  const canGrantBlog = !!tier.canCreateBlog;
  ...
  // Session 5.9 — new ability toggles for invite form.
  const [canManageBilling, setCanManageBilling] = useState(false);
  const [canManageIpd, setCanManageIpd] = useState(false);
  ...
  // Session 5.9 — new ability toggles for edit modal.
  const [editCanBilling, setEditCanBilling] = useState(false);
  const [editCanIpd, setEditCanIpd] = useState(false);
  const [editCanAnalytics, setEditCanAnalytics] = useState(false);
  const [editCanBlog, setEditCanBlog] = useState(false);
  ...
  const openEdit = (m: Member) => {
    ...
    setEditCanBilling(!!m.can_manage_billing);
    setEditCanIpd(!!m.can_manage_ipd);
    setEditCanAnalytics(!!m.can_view_analytics);
    setEditCanBlog(!!m.can_manage_blog);
    setEditCanSettings(!!m.can_manage_settings);
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.patch(`/team/${editing.email}`, {
        ...
        can_manage_billing: editCanBilling,
        can_manage_ipd: editCanIpd,
        can_view_analytics: editCanAnalytics,
        can_manage_blog: editCanBlog,
        can_manage_settings: editCanSettings,
  ...
  return (
    <>
      ...
      <View style={[styles.formCard, { marginBottom: 14 }]}>
        ...
        />
        <PermCheck
          label="Can manage Billing & Receipts"
          description="Open the Billing hub, record payments, generate receipts (reception / front-desk)."
          value={canManageBilling}
          onToggle={() => setCanManageBilling((v) => !v)}
          testID="team-billing"
        />
        <PermCheck
          label="Can manage IPD (admissions, beds, discharge)"
          description="Access in-patient module — admit, manage beds, write discharge summaries."
          value={canManageIpd}
          onToggle={() => setCanManageIpd((v) => !v)}
          testID="team-ipd"
        />
        <PermCheck
          label="Can view Analytics"
          description="Open the analytics dashboard (revenue, demographics, KPI charts)."
          value={canViewAnalytics}
          onToggle={() => setCanViewAnalytics((v) => !v)}
          testID="team-analytics"
        />
        {canGrantBlog ? (
          <PermCheck
            label="Can write Blog posts"
            description="Compose, edit and publish patient-education blog articles."
            value={canManageBlog}
            onToggle={() => setCanManageBlog((v) => !v)}
            testID="team-blog"
          />
        ) : (
          <View style={{ paddingVertical: 6 }}>
            <Text style={{ ...FONTS.body, fontSize: 11, color: COLORS.textSecondary, fontStyle: 'italic' }}>
              Blog access is currently disabled for your account, so you can't grant it to teammates.
            </Text>
          </View>
        )}
        <PermCheck
          label="Can edit Clinic Settings"
          description="Edit branding, fees, push templates, Google review template, etc."
          value={canManageSettings}
          onToggle={() => setCanManageSettings((v) => !v)}
      ...
      >
        <SafeAreaView style={styles.modalBackdrop} edges={['top', 'bottom']}>
          ...
          >
            ...
            <PermCheck
              label="Can manage Billing & Receipts"
              description="Open the Billing hub, record payments, generate receipts."
              value={editCanBilling}
              onToggle={() => setEditCanBilling((v) => !v)}
              testID="team-edit-billing"
            />
            <PermCheck
              label="Can manage IPD"
              description="Admit, manage beds, write discharge summaries."
              value={editCanIpd}
              onToggle={() => setEditCanIpd((v) => !v)}
              testID="team-edit-ipd"
            />
            <PermCheck
              label="Can view Analytics"
              description="Open the analytics dashboard (revenue, demographics)."
              value={editCanAnalytics}
              onToggle={() => setEditCanAnalytics((v) => !v)}
              testID="team-edit-analytics"
            />
            {canGrantBlog ? (
              <PermCheck
                label="Can write Blog posts"
                description="Compose, edit and publish blog articles."
                value={editCanBlog}
                onToggle={() => setEditCanBlog((v) => !v)}
                testID="team-edit-blog"
              />
            ) : (
              <View style={{ paddingVertical: 6 }}>
                <Text style={{ ...FONTS.body, fontSize: 11, color: COLORS.textSecondary, fontStyle: 'italic' }}>
                  Blog access is currently disabled for your account, so you can't grant it here.
                </Text>
              </View>
            )}
            <PermCheck
              label="Can edit Clinic Settings"
              description="Edit branding, fees, push templates, review template."
              value={editCanSettings}
              onToggle={() => setEditCanSettings((v) => !v)}
              testID="team-edit-settings"
            />
            <PermCheck
