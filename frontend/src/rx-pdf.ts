...
import api from './api';
import { buildWaMessage, type AutoWaContext } from './whatsapp-pdf';
...
export type ClinicSettings = {
  ...
  // ─── Rx Print Mode (offset-printed letterhead) ────────────────
  // When True, buildRxHtml() suppresses the app-rendered header AND
  // footer (logo, brand strip, QR, signature card, promise block,
  // dashed footer, watermark) and renders ONLY the dynamic body in
  // plain black ink. Designed for clinics that feed pre-printed
  // offset letterhead paper into the clinic printer — the body
  // aligns into the blank space below the printed header AND the
  // colour-ink usage drops to zero, so prints come out faster.
  rx_print_mode?: boolean;
  // Top padding (mm) when print mode is on; tunes where the body
  // begins so it lines up with the blank area beneath the pre-
  // printed letterhead. Default 40mm.
  rx_print_top_mm?: number;
  ...
  // Phase 5.2 — Google review CTA strip rendered in the Rx PDF
  // footer (QR + tagline). Populated by loadClinicSettings() from
  // GET /api/featured-reviews/cta.
  review_cta_url?: string;
  review_cta_maps_url?: string;
  review_cta_tagline?: string;
  review_cta_qr_svg_b64?: string;
  // Phase 5.20 — Auto-prompt the doctor to open WhatsApp with the
  // patient's chat pre-loaded after every Rx / Discharge / Med Cert
  // PDF is generated. Defaults to TRUE; owner can flip to FALSE in
  // Branding → Communications. Read from clinic_settings.
  whatsapp_auto_prompt_enabled?: boolean;
  /** Country code prefix for the patient's phone (default "+91"). */
  country_code?: string;
...
export async function buildRxHtml(rx: RxDoc, settings: ClinicSettings = {}): Promise<string> {
  ...
  // ─── Rx Print Mode (computed early) ────────────────────────────
  // Hoisted to the top of the builder so downstream colour choices
  // (QR ink, section header palette) can switch based on it BEFORE
  // any asset is generated. The full doc-string for this feature
  // lives next to the second printMode block below; this just
  // captures the boolean for early branching.
  const printMode = !!settings.rx_print_mode;
  ...
  try {
    qrSvg = await QRCode.toString(verifyUrl, {
      ...
      margin: 1,
      width: 180,
      // Print Mode: generate the QR in pure black so it doesn't pick
      // up the teal palette (the grayscale image filter we tried
      // first only turned the teal into a dark teal-gray which the
      // user flagged on Jun-13). Generating in black at the source
      // is the only reliable way to get a true black-on-white QR.
      color: printMode
        ? { dark: '#000000', light: '#FFFFFF' }
        : { dark: '#0A5E6B', light: '#FFFFFF' },
    });
  ...
  // ─── Rx Print Mode ─────────────────────────────────────────────
  // When the clinic feeds pre-printed offset letterhead into the
  // clinic printer, the app must NOT print its own header/footer
  // (logo, QR, promise, signature card, dashed footer, watermark)
  // and must render the body in plain black ink so the dynamic
  // content slots cleanly into the blank space below the printed
  // letterhead. `printMode` short-circuits every chrome block below.
  // `printTopMm` is the configurable top padding (mm) so the doctor
  // can dial in where the body begins.
  // letterhead; side/bottom margins kept generous
  // so the body never collides with the footer band on the printed
  // letterhead. */
  // `printMode` was already computed at the top of buildRxHtml so
  // it could drive the QR ink colour; we only resolve the top-
  // offset (mm) here next to the rest of the print-mode metadata.
  const printTopMm = Math.max(0, Math.min(120,
    Number.isFinite(settings.rx_print_top_mm as any)
      ? Number(settings.rx_print_top_mm)
      : 40
  ));
  ...
  // In Print Mode we KEEP the Patient Education + Need Help cards — the
  // doctor wants this clinically-useful content on every printout (just
  // in plain black ink). Only the top brand strip and the very bottom
  // dashed "ConsultUro stamp / Google review CTA" lines are stripped
  // out, because those are pure platform branding.
  const leftCol = [
    vitalsSection,
    ipssSection,
...
<style>
  ...
  body{
    ...
    color:${printMode ? '#000' : '#1A2E35'};
    font-size:11.5px;
    line-height:1.45;
    background:${printMode ? '#fff' : '#E5E9EC'};
  }
  ${printMode ? `
  /* ─── Print Mode overrides ───────────────────────────────────
     Forces every visible text/border colour to plain black so the
     printer only uses the black-ink cartridge (faster + cheaper).
     The body starts ${printTopMm}mm from the top to align with the
     blank area beneath the pre-printed offset letterhead.
     We KEEP the QR / Our Promise / Sanskrit / Signature footer
     row + the Patient Education + Need Help cards (per user
     request — these are clinically-useful content that must
     appear on every printout). We ONLY strip:
       • the top app-rendered brand strip (logo + doctor name +
         clinic strip + date/time meta) because the pre-printed
         letterhead already has this
       • the watermark (visual chrome)
       • the ConsultUro platform stamp at the very bottom
       • the optional Google-review CTA strip (pure branding) */
  *, *::before, *::after {
    color:#000 !important;
  ...
  /* QR SVG: regenerated upstream with dark:'#000000' so its dark
     modules already print black. Crucially we do NOT add a blanket
     "fill:#000" to the universal selector — that turned the QR's
     light <rect> modules black too and produced a solid black
     square in print mode (Dr. Joshi reported this Jun-15). The QR
     is now left untouched by the override and prints crisp B/W.
     Signature scan may have been saved with a coloured pen — force
     it to grayscale so it prints as plain black ink. */
  .signImg, .signature, img.signature { filter: grayscale(100%) contrast(140%) !important; }
  .watermark { display:none !important; }
  .head, .letterhead, .metaStrip { display:none !important; }
  .consulturo-stamp, .review-cta { display:none !important; }
  .rxSpacer { display:none !important; }
  /* Section header strips (Vitals · Chief Complaints · ...) render
     with a teal-on-white background by default — clear it so the
     title text reads as plain black on white, matching the
     pre-printed letterhead aesthetic. */
  .sech, .sechH, .sectionHeader, .sectionTitle {
    background:#fff !important;
    color:#000 !important;
    border-bottom:1px solid #000 !important;
  }
  .sec, .card { border-color:#000 !important; }
  ` : ''}
  .page{
    width: 210mm;
    min-height: 297mm;
    /* tighter top padding — pulls header up; matches user request.
       ...
       consultations.
       PRINT MODE: top padding bumped to rx_print_top_mm (default
       40mm) so the body slots into the blank area below the pre-
       printed offset letterhead; side/bottom margins kept generous
       so the body never collides with the footer band on the printed
       letterhead. */
    padding: ${printMode ? `${printTopMm}mm 14mm 20mm 14mm` : '12mm 14mm 10mm 14mm'};
    margin: 8mm auto;
    background: #fff;
  ...
  .brand p{
    margin:2px 0 0; color:#5E7C81; font-size:10.5px; line-height:1.4;
    /* Prevent long subline (e.g. "Consultant Urologist, Laparoscopic
       & Transplant Surgeon") from wrapping into the next paragraph
       when the brand block height collapses under flex centering. */
  }
  .meta{
    text-align:right; font-size:10.5px; color:#5E7C81;
    align-self:flex-start; flex:0 0 auto; padding-top:2px;
    white-space:nowrap;
  }
  ...
  /* ---- Patient summary band ----------------------------------------
     Switched from 5-col fr grid (which the WebKit print engine collapses
     in narrow A4 widths, causing PHONE / VISIT / REG.NO values to
     overlap into one string) to a 6-col explicit-percentage grid with
     ROW-LEVEL cells so each label/value pair keeps its own column.
     Patient + Age/Sex sit on the right, the operational IDs on the
     left, in two rows when content is dense. */
  .pd{
    background:#F4F9F9;
    ...
    margin-top:8px;
    display:grid;
    grid-template-columns: minmax(0, 1.5fr) minmax(0, 1.1fr) minmax(0, 1.2fr) minmax(0, 1.4fr) minmax(0, 1.2fr);
    column-gap:16px;
    row-gap:4px;
    position:relative; z-index:1;
  }
  ...
  .pd .v{
    color:#1A2E35; font-weight:600; font-size:11px;
  ...
  /* ---- Google Review CTA strip (Phase 5.2) ------------------------ */
  /* Sits inside the page-level dashed footer, BEFORE the ConsultUro
     stamp. QR is rendered as inline SVG (base64-encoded) so it stays
     crisp in print without a network dependency. */
  .review-cta{
    margin-top: 5px;
    display: flex;
    align-items: center;
...
<div class="page">
  ...
  <div class="foot">
    ...
    ${settings.review_cta_url ? `
    <div class="review-cta">
      ${settings.review_cta_qr_svg_b64
        ? `<img class="review-cta-qr" src="data:image/svg+xml;base64,${escapeHtml(settings.review_cta_qr_svg_b64)}" alt="Google Review QR"/>`
        : ''}
      <div class="review-cta-text">
        <div class="review-cta-line1">⭐ ${escapeHtml(settings.review_cta_tagline || 'Loved your visit? Leave a quick Google review 🙏')}</div>
        <div class="review-cta-line2">Scan the QR or visit: ${escapeHtml((settings.review_cta_url || '').replace(/^https?:\/\//, ''))}</div>
      </div>
    </div>` : ''}
...
export async function loadClinicSettings(): Promise<ClinicSettings> {
  ...
  const resolved = { ...cs, ...hp,
    ...
    // suppress its full chrome when the clinic uses pre-printed
    // offset letterhead. Both fields fall back to clinic_settings
    // (homepage settings never define them).
    rx_print_mode: cs.rx_print_mode ?? hp.rx_print_mode ?? false,
    rx_print_top_mm: cs.rx_print_top_mm ?? hp.rx_print_top_mm ?? 40,
    ...
    // Google review CTA bundle for the Rx-PDF footer.
    review_cta_url: cta.review_url || '',
    review_cta_maps_url: cta.maps_url || '',
    review_cta_tagline: cta.tagline || '',
    review_cta_qr_svg_b64: cta.qr_svg_b64 || '',
    // WhatsApp auto-prompt config — owner-toggleable. Defaults TRUE
    // when the field is absent so first-time installs get the
    // post-PDF prompt by default.
 ...
 */
/**
 * Prompts the doctor to also open the patient's WhatsApp chat with a
 * pre-filled follow-up message after the Rx PDF has been shared.
 *
 * Why this is its own helper inside rx-pdf.ts (not in
 * whatsapp-pdf.ts): we want this to ALSO fire after the legacy
 * `Print.printToFileAsync → Sharing.shareAsync` and Web-Share-API
 * paths inside `sharePrescriptionPdf` — which don't go through
 * `sharePdfFromHtml` and therefore can't reuse `sharePdfThenWhatsApp`.
 * Centralising the prompt logic here keeps the UX identical across
 * all three Rx share paths.
 */
async function _maybePromptWhatsAppRx(rx: RxDoc, settings?: ClinicSettings): Promise<void> {
  const enabled = (settings?.whatsapp_auto_prompt_enabled !== false);
  if (!enabled) return;
  const digits = toInternationalDigits(
    (settings?.country_code as any) || '+91',
    rx.patient_phone || '',
  );
  if (!digits) return;
  const ctx: AutoWaContext = {
    patientName: rx.patient_name || null,
    patientPhone: rx.patient_phone || null,
    countryCode: (settings?.country_code as any) || '+91',
    docKind: 'rx',
    followUpDate: rx.follow_up || null,
    doctorName: (settings?.doctor_name as any) || null,
    enabled,
  };
  ...
  const openIt = async () => {
    try {
      const waScheme = `whatsapp://send?phone=${digits}&text=${encodeURIComponent(buildWaMessage(ctx))}`;
      if (Platform.OS !== 'web') {
        const supported = await Linking.canOpenURL(waScheme);
        if (supported) { await Linking.openURL(waScheme); return; }
      }
      ...
      try {
        ...
        if (FileCtor && (navigator as any)?.canShare) {
          ...
          if ((navigator as any).canShare({ files: [file] })) {
            ...
            // Pop a follow-up "open WhatsApp chat?" prompt so the
            // doctor doesn't have to switch apps + find the contact.
            await _maybePromptWhatsAppRx(rx, settings);
            return;
          }
        }
      } catch (e: any) {
