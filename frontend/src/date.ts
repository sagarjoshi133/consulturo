...
/** IST date+time in clinical-readable form — "12 Jun 2026, 04:30 PM IST".
 *  Designed for clinical UIs (IPD tabs, vitals, rounds, transfers,
 *  audit trail) where unambiguous local time matters more than
 *  compactness. Always parses backend timestamps as UTC (even when
 *  the trailing Z is missing) and then formats in Asia/Kolkata to
 *  prevent the device timezone from skewing the displayed time. */
export function formatISTShort(input?: string | Date | null): string {
  if (!input) return '';
  const d = parseBackendDate(input);
  if (!(d instanceof Date) || isNaN(d.getTime())) return '';
  try {
    ...
    const dayPeriod = (get('dayPeriod') || '').toUpperCase().replace(/\./g, '').replace(/\s/g, '');
    return `${day} ${month} ${year}, ${hour}:${minute} ${dayPeriod} IST`;
  } catch {
    // Older JS engines without full ICU — manual +5:30 offset.
    const ms = d.getTime() + 5.5 * 3600 * 1000;
    const s = new Date(ms).toISOString();
    const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    const yy = Number(s.slice(0, 4));
    const mo = months[Number(s.slice(5, 7)) - 1] || s.slice(5, 7);
    const dd = s.slice(8, 10);
    const hh24 = Number(s.slice(11, 13));
    const mm = s.slice(14, 16);
    const ampm = hh24 >= 12 ? 'PM' : 'AM';
    const hh12 = ((hh24 + 11) % 12) + 1;
    const hhPad = hh12 < 10 ? `0${hh12}` : String(hh12);
    return `${dd} ${mo} ${yy}, ${hhPad}:${mm} ${ampm} IST`;
  }
}
