            ...
            <View key={p.user_id || p.email}>
              <PersonRow
                ...
                blogToggle={
                  tier.isSuperOwner
                    ? {
                        // Default-True semantics: primary_owners have
                        // blog access unless explicitly revoked (the
                        // backend treats a missing flag as TRUE).
                        // Super-owner row is locked ON.
                        value:
                          p.role === 'super_owner'
                            ? true
                            : (p.can_create_blog !== false),
                        disabled: p.role === 'super_owner',
                        onChange: async (v: boolean) => {
                          if (!p.user_id || p.role === 'super_owner') return;
                          setBusy(true);
